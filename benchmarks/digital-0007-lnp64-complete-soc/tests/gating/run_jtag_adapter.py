...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: run_jtag_adapter.py OUTPUT")
    rtl = Path(sys.argv[1]).resolve() / "rtl"
    ...
    with tempfile.TemporaryDirectory(prefix="lnp64-jtag-") as temp:
        executable = Path(temp) / "jtag-contract.vvp"
        command = ["iverilog", "-g2012", "-s", "tb_jtag_contract", "-o", str(executable)]
        command += [str(path) for path in sources]
        command += [str(Path(__file__).with_name("tb_jtag_contract.sv"))]
