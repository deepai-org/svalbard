...
| Programmable leaf calibration | [`reference_level_receiver/run_bias_sweep.py`](../../ip/blocks/analog/wireline_serdes/reference_level_receiver/run_bias_sweep.py) and its physical checker | Reuses one schematic/PEX estimator across realizable bias codes, requires union coverage of every declared PVT environment, hashes the sweep runner and DUT, and audits generated layout device widths/fingers against extracted devices | Code coverage does not transfer to a parent whose pulse width, slew, source impedance, history, or load falls outside the leaf campaign; the current PCIe v5 parent is retained proof of that boundary. |
