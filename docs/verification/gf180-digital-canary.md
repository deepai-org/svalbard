...
4. The scan netlist closes through a second RTL-to-GDS flow.
5. Exhaustive one-cycle patterns must detect both stuck-at values on every derived cell-output fault site.
...
This qualifies a generic core-flow canary only. It does not qualify the public PDK for fabrication, project RTL, pads, independent ATPG or transition-fault coverage, SDF, package-aware IR/EM, or provider precheck.
