...
| Autonomous clock source | Seven complete 1.25 GHz folded oscillator parents are independently 0-DRC, unique-LVS and exact full-RC PEX. The 280-case no-IC/no-uic bank sweep passes the required target in 5/5 environments (183 valid cases); continuous +/-2% design margin passes 3/5. Three additional split-control parents are independently physical and their focused 240-case extracted screen covers the complete +/-2% band in both formerly open hot slow-device environments (84 valid cases). The older twelve-parent 2.5 GHz full-rate experiment is physically legal but reaches target in only 2/5 environments. The two-input selector and balanced sixteen-leaf hierarchy retain their prior full-rate isolation/handoff evidence | Split control is a physically proven margin mechanism, not yet a five-environment qualification. Realizable dual-bias generation/calibration, selected-bank/tree/power-control composition, statistical startup, phase noise, supply pushing, mismatch, divider loading and closed PLL remain unclosed |
