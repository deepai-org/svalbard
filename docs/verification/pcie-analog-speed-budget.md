...
| Autonomous clock source | A minimum-subset proof selects two complete 1.25 GHz split-control folded parents from three 0-DRC, unique-LVS, full-RC candidates. Their 293/400-case no-IC/no-uic union has two unique PEX identities and continuous target plus +/-2% design-band coverage in 5/5 environments; no single candidate exceeds 3/5. Seven fixed-control parents and a passing ten-parent 610/880 aggregate remain corroborating evidence. The older twelve-parent 2.5 GHz full-rate experiment reaches target in only 2/5. The physically closed two-input selector matches the selected bank; the sixteen-leaf hierarchy is no longer required | Bare-bank deterministic PVT range and candidate minimality are closed. Realizable dual-bias generation/calibration, two-parent selector/power composition, statistical startup, phase noise, supply pushing, mismatch, divider loading and closed PLL remain unclosed |
