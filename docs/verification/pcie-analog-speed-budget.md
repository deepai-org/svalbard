...
| PI-clocked 2.5 GT/s lane | The zero-DRC, unique-LVS 8,625R/5,034C parent routes the PI, two-stage clock restorer, termination, RX, limiter, sampler, converters, and split capture. TT and SS/passive extracted clock-load tests each pass 4/4 restorer biases. A 24-bit combined-stress nominal screen passes adjacent 200 and 300 ps conversion offsets after observing 120 ps beyond capture close; worst passing sampler, converter, and capture margins are 510.617 mV, 306.147 mV, and 1.26862 V. The five-environment composed replay is only 1/5. | GF180 speed is adequate for the isolated clock chain, but held-state capture fails FF/cold and dynamic decisions fail SS. Redesign and exact-parent PVT replay are required before autonomous-CDR composition. |
