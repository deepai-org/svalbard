...
| Dual-edge sense/write pulse generator | A real four-profile, one-hot programmable transistor circuit passes the full schematic screen with realizable coverage of TT, FF/cold, FF/hot, SS/cold, and SS/hot. The revised write path uses a reset-dominant cross-coupled-NOR interval latch instead of asking a sub-200 ps combinational glitch to drive the output taper. Its current 414-MOS, 456.2 by 285 um stacked-phase layout is 0-DRC, uniquely pin-resolved LVS-matched, and extracts to 8,021R/5,459C. Exact nominal PEX now produces both output classes: SENSE is 563.10 ps and rail-valid, while WRITE reaches 2.857 V with 258.23 ps width, 516.62 ps leading-edge delay, and 62.83 mA supply current. This is material recovery from the former missing write pulse, but it remains a failure against the 100--220 ps width, non-overlap, and 3.05 V nominal-high requirements. A switched-capacitance trim was explicitly falsified: it moved the leading edge by only about 1 ps while degrading latch recovery. | Shorten and rail-restore the extracted write event with an edge-selective physical mechanism, preserve schematic PVT coverage, then run full extracted PVT and compose with the rail converter and real capture gates. |
