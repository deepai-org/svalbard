...
| Externally clocked lane spine | At 1.25 GBd, the exact TX/termination/RX/restorer/sampler/converter/capture stack passes 5/5 environments and 160/160 PRBS bits under simultaneous bounded channel, jitter, duty, and rail-ripple stress. At 2.5 GT/s, the 7,900R/4,804C externally clocked parent passes 5/5 representative environments over 24-bit PRBS7. The 8,625R/5,034C zero-DRC, unique-LVS parent additionally routes the PI and two-stage clock restorer. Its TT and SS/passive nonlinear clock-load chains each pass 4/4 biases. Correct post-write scoring closes adjacent 200 and 300 ps offsets in an eight-point nominal 24-bit screen; worst passing sampler, converter, and final margins are 510.617 mV, 306.147 mV, and 1.26862 V. The composed five-environment replay passes only nominal. FF/cold retains over 0.811 V sampler and 2.327 V converter margin but collapses below 2 mV after capture closes; FF/hot fails long-pattern recurrence; SS cases lose dynamic decisions. | Redesign held-state capture for FF/cold and localize the SS dynamic data/clock boundary, then replay the full 24-bit PVT matrix. After that add mismatch/noise/metastability, realized pulse/bias generation, package/pad/channel EM, substrate/PDN aggression, fill, EM/IR, and provider signoff models. |
...
1. Close post-write capture regeneration and dynamic PI/sampler operation in
   every representative PVT environment, then replay the 24-bit exact-parent
   matrix with adjacent calibration-code margin.
