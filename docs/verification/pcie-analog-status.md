...
| Dual-edge sense/write pulse generator | A real four-profile, one-hot programmable transistor circuit passes the 20-case schematic screen with realizable coverage of TT, FF/cold, FF/hot, SS/cold, and SS/hot. Its current 392-MOS, approximately 546 by 285 um stacked-phase layout is 0-DRC and uniquely pin-resolved LVS-matched. Per-phase restoration closes the former odd P08 failure at 2.80/0.29 V, both phase selectors switch, and a rebalanced taper raises exact-PEX WRITE to 2.52 V even and 1.87 V odd. Exact nominal evidence remains failed: sense widths are only 147.57/217.06 ps and write widths 224.10/23.74 ps. An exact all-profile scan finds no existing code that reaches the 450--650 ps sense window. | Replace or extend the physical sense-window architecture, then finish the loaded output taper without relaxing the 450--650 ps sense, 100--220 ps write, 500--700 ps delay, 75 mA, or rail-margin contracts; add an independent device-property checker, run full extracted PVT, and compose with the rail converter and real capture gates. |
