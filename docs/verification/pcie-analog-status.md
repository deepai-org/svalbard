...
| Dual-edge sense/write pulse generator | A new HCLK-based topology removes the failed global tap selector/latch network and reduces the stacked-phase macro from 428 to 128 MOS and from 499.6 to 175.4 um width. It is zero-DRC, uniquely pin-resolved LVS-matched, and 3,408R/2,477C full-RC extracted. Exact PEX produces complete dual-phase SENSE and WRITE waveforms at TT, FF/125 C, and SS/125 C while current falls to 23.501--41.172 mA. Sense widths are 539.69--598.30 ps. TT/FF write widths are still too broad at 336.47--349.84 ps and start before the external SENSE fall; SS/hot write widths are 119.84/123.13 ps but reach only 1.922/1.926 V. Thus all three focused cases remain contract failures and the complete five-environment PVT campaign is not promoted. The former 428-MOS nominally passing checkpoint remains immutable evidence rather than being overwritten. | Calibrate write leading edge, width, and slow-hot restoration on the compact topology; then run the full exact 20-case matrix and compose the same extracted macro with the converter and regenerative capture gates. |
