...
| PLL/VCO/divider | The dual-edge architecture requires a 1.25 GHz oscillator. A checked minimum-subset search selects the `fast` and `gain` split-control parents from three independently 0-DRC, unique-LVS, full-RC candidates. The selected 293/400-case no-IC/no-uic union has two unique PEX identities and continuous target plus +/-2% design-band coverage in 5/5 environments; no single candidate exceeds 3/5. The reused physical dual 5-bit R-2R DAC is qualified for VCO bias: 160/160 DC cases, all requested controls within 29.2 mV, 65.3 mV worst step, and 5/5 worst-carry 1 pF cases settled within 18 uV by 50 ns. The selected two-DAC/two-VCO/two-input-selector parent is now zero-DRC, uniquely LVS-matched, and full-RC extracted with 3,773 resistors and 1,220 capacitors. Its compensated clock routes, local bias routes, shared references, and supply spines are real parent geometry. A separate twelve-parent 2.5 GHz overspeed experiment is physically clean but covers only 2/5 and is retained as falsification evidence | Electrically qualify the complete parent PEX with realizable code calibration, startup/shutdown, inactive isolation, reference loading, and break-before-make sequencing; then close statistical startup/mismatch, phase noise/supply sensitivity, divider and closed PLL |
...
2. Electrically qualify the routed two-DAC/two-VCO/selector parent using the
   measured target-to-code map, shared reference loading, startup/power-off,
   inactive isolation, and safe sequencing; then build the divider
