...
| CML-to-CMOS boundary | The original programmable-tail converter is zero-DRC, uniquely LVS-matched, and 2,048R/1,340C full-RC extracted, but its 700--900 ps useful window cannot serve a 400 ps serial UI reliably. The 190 x 160 um StrongARM-style replacement is zero-DRC, uniquely LVS-matched, and 2,087R/1,256C full-RC extracted. All 10/10 targeted exact-PEX environments pass with 510.34 mV minimum fixed-point logic margin and 9.592 mA maximum current. A separate exact-PEX data-age grid proves that previous data is retained through 500 ps, all three limiting environments resolve new data by 760 ps, and all exceed 150 mV margin by 770 ps in the 800 ps lane cycle. | The lower input capacitance is now composed into the routed PI/RX parent, but one-lane-cycle output timing leaves little slow/hot slack. Add a valid-window retimer or reschedule the acquisition/write boundary, then close noise/mismatch, metastability tails, extracted PDN/substrate, fill, and reliability. |
...
| Externally clocked lane spine | At 1.25 GBd, the exact TX/termination/RX/restorer/sampler/converter/capture stack passes 5/5 environments and 160/160 PRBS bits under simultaneous bounded channel, jitter, duty, and rail-ripple stress. At 2.5 GT/s, the 7,900R/4,804C externally clocked parent passes 5/5 representative environments over 24-bit PRBS7. The PI-clocked fast-converter parent is separately zero-DRC, unique-LVS, and 8,717R/4,874C full-RC extracted. Under the same 24-bit PRBS7, 6 ohm/leg + 1 pF channel, 30 ps jitter, 47% duty, and 20 mV rail-ripple stress it passes TT, FF/cold, FF/hot, and SS/passive. The limiting passing final margin is 685.201 mV. SS/hot is a preserved failure: even and odd lanes carry the requested word at different integer data ages and no common age passes. | Add an explicit retiming or revised non-overlapping sense/write schedule and require 5/5 exact-parent closure. Then add mismatch/noise/metastability, realized pulse/bias generation, package/pad/channel EM, substrate/PDN aggression, fill, EM/IR, and provider signoff models. |
...
1. Resolve the routed fast PI/RX parent's SS/hot mixed-lane data age with an
   explicit valid-window retimer or revised non-overlapping acquisition/write
   schedule, then require 5/5 exact-parent closure with adjacent timing margin.
2. Realize upstream clock and bias distribution, local decoupling, and the
