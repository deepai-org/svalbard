...
| PLL/VCO/divider | The dual-edge architecture requires a 1.25 GHz oscillator. A checked minimum-subset search selects the `fast` and `gain` split-control parents from three independently 0-DRC, unique-LVS, full-RC candidates. The selected 293/400-case no-IC/no-uic union has two unique PEX identities and continuous target plus +/-2% design-band coverage in 5/5 environments; no single candidate exceeds 3/5. The physical dual 5-bit R-2R DAC now uses a 2.0 V reference and passes 160/160 DC cases plus 5/5 settling cases; all requested controls are within 32.6 mV, the lowest high endpoint is 1.885 V, and worst reference power is 1.462 mW. The selected two-DAC/two-VCO/two-input-selector parent is zero-DRC, uniquely LVS-matched, and full-RC extracted with 3,872 resistors and 1,287 capacitors. Its exact PEX passes 4/7 nominal candidates, 10/35 PVT candidates covering 5/5 environments, and break-before-make handoff from 1.2575 to 1.2452 GHz with 3.46 mV dead-interval residue. One aggregate report binds every pass to the same PEX and proves three selected codes of rail headroom. The twelve-parent 2.5 GHz overspeed experiment remains failed falsification evidence | Close statistical startup/mismatch, phase noise, supply/reference sensitivity, divider loading, calibration control, and the closed PLL |
...
2. Close statistical startup/mismatch, phase noise, and supply/reference
   sensitivity on the qualified routed VCO parent; then build the divider and
   PLL with external-clock bypass and observable divided clock/lock outputs.
