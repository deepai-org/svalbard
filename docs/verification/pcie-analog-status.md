...
2. Place, route, extract, and re-simulate the combined VCO-bank/restorer/divider
   parent; then close statistical startup/mismatch, phase noise, and combined
   PDN/aggressor sensitivity, implement the remaining reference ratio, and
   build the PLL with external-clock bypass and observable divided clock/lock
   outputs.
