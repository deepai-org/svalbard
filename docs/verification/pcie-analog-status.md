...
| Dual-edge sense/write pulse generator | A real four-profile, one-hot programmable transistor circuit passes the 20-case schematic screen with one realizable profile covering each of TT, FF/cold, FF/hot, SS/cold, and SS/hot. The restoring-selector redesign replaces the incompatible ratioed start/end paths. Its current 396-MOS, approximately 544 by 285 um stacked-phase layout is 0-DRC and uniquely pin-resolved LVS-matched. Exact nominal full-RC evidence is intentionally retained as failed: local tap-buffer placement restores the selected start/end nodes and WST/WET switching, but the approximately 115 ps internal write interval is lost in the distributed output taper; WB0 reaches only 1.40 V and WRITE 0.49 V. | Retaper and compact the write-core/output chain without relaxing the 450--650 ps sense, 100--220 ps write, 500--700 ps delay, 75 mA, and rail-margin contract; add an independent device-property checker, then run full extracted PVT and compose with the rail converter and real capture gates. |
