...
| Dual-edge sense/write pulse generator | A real four-profile, one-hot programmable transistor circuit passes the 20-case schematic screen with one realizable profile covering each of TT, FF/cold, FF/hot, SS/cold, and SS/hot. The current 372-MOS stacked-phase layout is 0-DRC and uniquely pin-resolved LVS-matched. Exact full-RC nominal simulation is intentionally retained as failed evidence: phase stacking and functional placement restore the delay/tap chain and strong internal sense switching, but extracted selector/output capacitance still distorts pulse width, misses the final sense rail guardband, and prevents reliable write restoration. | Finish the selector/restoration physical architecture without relaxing the 450--650 ps sense, 100--220 ps write, 500--700 ps delay, 75 mA, and rail-margin contract; add an independent device-property checker, then run full extracted PVT and compose with the rail converter and real capture gates. |
