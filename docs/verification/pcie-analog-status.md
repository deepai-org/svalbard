...
| Externally clocked lane spine | The selected serializer/TX full-RC extraction drives an explicit AC-coupled provisional boundary and fresh termination/RX/sampler PEX. The nominal sampler composition passes 13/16 phases and its selected phase passes 5/5 environments. Two fresh CML-to-CMOS PEX instances plus the compacted split-clock capture PEX extend this same parent to stable parallel CMOS: 4/4 nominal conversion offsets and 5/5 representative environments pass, with 2.371 V worst converter margin, 2.363 V worst parallel-output margin, and 23.05--34.49 mA total current. | Add a longer PRBS scoreboard; sweep channel loss, jitter, duty distortion, mismatch and simultaneous supply/substrate aggression. Replace the provisional boundary with selected pad/ESD/package/channel models. |
...
1. Extend the closed extracted 1.25 GBd TX-to-parallel-CMOS spine with a longer
   PRBS scoreboard and bounded channel loss, clock jitter/duty distortion,
   mismatch and simultaneous supply/substrate aggression. Reconcile complete
   path power before extending more autonomous leaves.
