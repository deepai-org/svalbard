...
| Dual-edge sense/write pulse generator | The compact HCLK-based topology removes the failed global tap selector/latch network and reduces the stacked-phase macro from 428 to 128 MOS and from 499.6 to 175.4 um width. It is zero-DRC, uniquely pin-resolved LVS-matched, and 3,492R/2,531C full-RC extracted. A local delayed-step interval generator now gives an exact-PEX TT contract pass: 591.87/583.98 ps SENSE, 206.87/213.84 ps WRITE, 605.54 ps write delay, 13.67 ps non-overlap, valid rails, and 40.669 mA. FF/125 C also meets all measured timing and SENSE-rail requirements, but WRITE reaches only 2.687/2.692 V against the 2.72 V minimum. SS/125 C has valid 540.98/540.58 ps SENSE pulses but the write restoration chain does not propagate a complete pulse. The focused result is therefore 1/3 environments, and the complete five-environment matrix is deliberately not promoted. | Add a realizable slow-hot profile at a full-width state before interval formation and recover the final FF/hot 28--33 mV of WRITE swing without violating the 220 ps width limit; then run all 20 exact-PVT profile/environment cases and compose converter + pulse generator + regenerative capture in one routed, extracted parent. |
| Local complementary capture-clock bridge | The physical WRITE-to-complementary-clock consumer is zero-DRC, uniquely LVS-matched, and full-RC extracted (96 raw fingers, 898R/560C).  Its exact PEX, driving the exact independently clocked capture PEX, passes 5/5 public-model corners with a 200 ps WRITE stimulus, correct alternating capture polarity, rails within 250 mV of supplies, at most 118.66 ps signed complementary skew, and at most 18.5 mA average current.  The limiting SS/125 C, 2.97 V positive clocks are 2.787/2.760 V; this is a local gate with only 40 mV margin to the deliberately conservative 2.72 V rail screen. | Drive it from the exact pulse-generator PEX and compose it with the direct-regenerative lane parent.  Preserve the present pulse-generator slow/hot failure rather than hiding it behind the bridge's ideal WRITE isolation stimulus. |
