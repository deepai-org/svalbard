...
| Dual-edge sense/write pulse generator | The reset-dominant interval-latch topology is now a 422-MOS, 499.6 by 285 um stacked-phase layout with 0 DRC, unique pin-resolved LVS, and 9,045R/6,110C exact extraction. Nominal exact PEX passes every timing and current limit: EVEN/ODD sense 574.57/602.71 ps, write 215.74/198.40 ps, delay 677.64 ps, non-overlap 103.07 ps, phase spacing 380.72 ps, and 68.61 mA. It remains failed because WRITE peaks are 3.002/2.906 V and sense lows are 0.253/0.313 V. Extracted path analysis reports 27.98 ohm worst final-PMOS source resistance after the latest PDN/access work, improved from 32.15 ohm. Larger final banks, feed-forward pull-ups, an earlier D07 epoch, a fractional transmission-gate epoch, switched-capacitance trim, and blind extra access columns were each falsified for current, timing, rail, DRC, or LVS reasons. The current 20-case schematic profile matrix also has no complete environment coverage, superseding the earlier passing claim. | Close local source/output resistance or add non-contending local rail restoration, rebuild full schematic profile coverage under 75 mA, then run full extracted PVT and compose with the rail converter and actual capture gates. |
...
1. Close the current 0-DRC/unique-LVS programmable sense/write pulse generator
   in exact PEX, then place it after the physically closed
