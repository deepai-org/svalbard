...
| CML-to-CMOS boundary | The original programmable-tail converter is zero-DRC, uniquely LVS-matched, and 2,048R/1,340C full-RC extracted, but exact-parent diagnostics show its 700--900 ps useful window cannot serve a 400 ps serial UI reliably. A new StrongARM-style schematic passes 10/10 targeted environments at a fixed 120 ps qualification point with 527 mV minimum logic margin and one 800 ps lane-cycle latency. | Implement the compact symmetric replacement layout, then require DRC, unique LVS, full-RC qualification, parent integration, noise/mismatch, and metastability evidence. |
...
1. Physically close the sub-400 ps CML-to-CMOS replacement, integrate both
   instances into the routed PI/RX parent, and replay the 24-bit exact-parent
   matrix in every representative PVT environment with adjacent timing margin.
