...
| Serializer/TX | The standalone 2:1 CML mux remains a zero-DRC, unique-LVS, 267R/85C test structure, but arbitrary changing words falsified its high-capacitance TX boundary.  The selected clock-steered serializer/TX parent is zero-DRC, uniquely LVS-matched, extracts to 1,081R/614C, and passes 35/35 exact-PEX changing-word aperture cases in 5/5 environments at both 1.25 and 2.5 GT/s.  Worst signed center margin is 0.56308/0.5289654 V and selected current is 4.906--7.225/8.555--10.125 mA respectively, with at least 60 ps setup and hold. | Jitter/duty distortion and mismatch, realizable bias control, pad/ESD/package load, then externally clocked PRBS lane composition |
| Externally clocked lane spine | The selected serializer/TX full-RC extraction drives an explicit AC-coupled provisional boundary and fresh termination/RX/sampler PEX. The nominal sampler composition passes 13/16 phases and its selected phase passes 5/5 environments. Two fresh CML-to-CMOS PEX instances plus the compacted split-clock capture PEX extend this same parent to stable parallel CMOS: 4/4 nominal conversion offsets and 5/5 representative environments pass, with 2.371 V worst converter margin, 2.363 V worst parallel-output margin, and 23.05--34.49 mA total current. A 64-bit PRBS7-prefix run then passes 4/4 exact-PEX cases and 224/224 scored serial bits across baseline, a 6-ohm/leg + 1-pF differential two-section RC proxy, 40-ps peak deterministic TX jitter with 47% duty, and combined stress; worst nominal converter/capture margins are 2.975/3.165 V. | Add a denser combined PVT/channel/jitter/duty matrix, mismatch and simultaneous supply/substrate aggression. Replace the provisional proxy with selected pad/ESD/package/S-parameter models. |
...
development, 30--40% of a functioning integrated PCIe analog interface, and
...
1. Extend the initial 64-bit exact-PEX lane stress into a denser combined
   PVT/channel/jitter/duty matrix, then add mismatch and simultaneous
   supply/substrate aggression. Reconcile complete-path power before extending
   more autonomous leaves.
2. Repeat the lane at externally clocked 2.5 GT/s, including the
