...
| CML-to-CMOS boundary | The original programmable-tail converter is zero-DRC, uniquely LVS-matched, and 2,048R/1,340C full-RC extracted, but its 700--900 ps useful window cannot serve a 400 ps serial UI reliably. The new 190 x 160 um StrongARM-style replacement is zero-DRC, uniquely LVS-matched, and 2,172R/1,311C full-RC extracted. All 10/10 targeted exact-PEX environments pass at the fixed 120 ps point with 546.42 mV minimum logic margin, 9.964 mA maximum average current, and one 800 ps lane-cycle latency. | Regenerate the routed PI/RX parent with the exact fast macro and replay PVT; then add noise/mismatch, metastability-tail, extracted-PDN/substrate, fill, and reliability evidence. |
...
| Externally clocked lane spine | At 1.25 GBd, the exact TX/termination/RX/restorer/sampler/converter/capture stack passes 5/5 environments and 160/160 PRBS bits under simultaneous bounded channel, jitter, duty, and rail-ripple stress. At 2.5 GT/s, the 7,900R/4,804C externally clocked parent passes 5/5 representative environments over 24-bit PRBS7. The 8,625R/5,034C zero-DRC, unique-LVS parent additionally routes the PI and two-stage clock restorer. Its TT and SS/passive nonlinear clock-load chains each pass 4/4 biases. Corrected qualification/write scoring closes adjacent 200 and 300 ps offsets nominally and passes TT, FF/cold, and SS/passive in the five-case exact-parent replay. FF/hot and SS/hot fail before capture at the old converter boundary; edge skew, sampler loading, and lane-delay diagnostics do not create a common correct window. | Replace both old converters with the physically closed fast macro, rebuild the exact routed parent, and replay the 24-bit PVT matrix. Then add mismatch/noise/metastability, realized pulse/bias generation, package/pad/channel EM, substrate/PDN aggression, fill, EM/IR, and provider signoff models. |
...
1. Integrate both physically closed sub-400 ps CML-to-CMOS instances into the
   routed PI/RX parent and replay the 24-bit exact-parent matrix in every
   representative PVT environment with adjacent timing margin.
