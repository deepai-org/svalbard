...
| Dual-edge sense/write pulse generator | A real four-profile, one-hot programmable transistor circuit passes the 20-case schematic screen with realizable coverage of TT, FF/cold, FF/hot, SS/cold, and SS/hot. Its current 446-MOS, 580.8 by 285 um stacked-phase layout is 0-DRC and uniquely pin-resolved LVS-matched. Static CMOS profile selection and local CT/ST restoration replace the former threshold-losing selector, and explicit parallel one-finger tri-state stacks make the schematic physically faithful. The latest 7,872R/5,263C exact nominal extraction remains failed: the even NOR pulse propagates but yields only about 332 ps of final sense width, the odd NOR pulse peaks at only 1.085 V and is filtered, and neither final write output crosses threshold. | Stabilize the dynamic odd NOR/edge-storage path and retaper the first write buffers without relaxing the 450--650 ps sense, 100--220 ps write, 500--700 ps delay, 75 mA, or rail-margin contracts; then run full extracted PVT and compose with the rail converter and real capture gates. |
