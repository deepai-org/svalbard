...
| Dual-edge sense/write pulse generator | The 193.3-um local-replica revision is zero-DRC, unique-LVS, and exact-nominal-PEX passing. WRITE timing now derives from a locally loaded `SB1` replica instead of the extracted SENSE consumer, which earns three passing sustained composed corners. Its present PVT control is static: `SEL0..SEL3` are pin-load anchors, not a realized profile mechanism. A 12-um fixed SENSE-NMOS counterfactual improved a local low rail but erased WRITE at multiple corners because of added preceding-stage load. | Create a physically effective, observable corner-selectable full-swing timing/drive state, its calibration decision, and a 5/5 PEX proof. |
...
| Pulse → bridge → direct-regenerative capture boundary | Fresh pulse PEX plus byte-bound bridge and lane PEX removes every ideal timing source at the boundary (only an upstream recovered rail-clock and static RX input remain). The settled 5--8 ns, two-cycle static-input screen passes 3/5: TT, FF/hot, and SS/cold. FF/cold has 3.379/3.396 V WRITE but only 3.311/3.296 V bridge clocks versus the 3.380 V criterion; SS/hot has 49/60 mV WRITE and no periodic capture event. | Add the real calibration path and repair both FF/cold bridge-drive and SS/hot reset mechanisms, then repeat this 5-corner gate before a routed parent, PRBS/channel, or system CDR closure. |
...
1. Extend the nominally closed, 0-DRC/unique-LVS programmable sense/write pulse
   generator from TT-only to 5/5 exact-PVT coverage, then place it after the physically closed
   PI/restorer/rail-converter path, drive the now-physical local capture-clock
   bridge and actual extracted direct-regenerative clock gates, and re-establish
   5/5 exact-parent closure.
