...
| CML-to-CMOS boundary | Keep one instance per required decision lane | The routed parent through dual CMOS capture passes 5/5 combined-stress environments and exposes a 750 ps output-validity bound. The following storage/retiming architecture and its remaining 50 ps same-lane margin still require composition. |
...
|---|---|---|
| GF180 can switch fast enough for a constrained 2.5 GT/s lane | Extracted TX and one routed termination-through-capture receive parent pass the bounded 2.5 GT/s core matrix; a 2.5 GHz VCO architecture did not. | Plausible for the constrained core, not proven at the pins. Retain 1.25 GHz dual-edge operation and require selected pad/ESD/package/channel and recovered-clock composition. |
