...
| CML-to-CMOS boundary | Keep one instance per required decision lane | It is the intended power/complexity boundary.  The present static-CML deserializer should not be assumed to be the final storage architecture. |
...
|---|---|---|
| GF180 can switch fast enough for a constrained 2.5 GT/s lane | Extracted TX, RX, sampling and phase-control leaves cross their local speed boundaries; a 2.5 GHz VCO architecture did not. | Plausible, not proven end to end.  Retain 1.25 GHz dual-edge operation and require an extracted full-lane eye before further optimization. |
