#!/usr/bin/env python3
"""Fail closed on physical closure of the active Wi-Fi RF NFET coupon."""
...
def main() -> None:
    ...
    device_count = len(re.findall(
        r"^X\d+\s+\S+\s+\S+\s+\S+\s+\S+\s+nfet_03v3\s+"
        r"[^\n]*\bw=4u\s+l=0\.28u$", pex, re.MULTILINE))
    result = {
        "schema_version": 1,
        "claim": "wifi_2p4g_lna_nfet_array_characterization_coupon",
        ...
        "nfet_03v3_4u_028u_count": device_count,
        ...
        "schematic_source_sha256": digest(args.source / "rf_nfet_array_coupon.spice"),
    ...
    passed = (result["drc_error_count"] == 0 and result["lvs_unique"]
              and device_count == 16 and result["pex_resistor_count"] >= 3
              and result["pex_capacitor_count"] >= 8
              and result["layout_image_bytes"] >= 15_000)
    result["result"] = "pass" if passed else "fail"
    args.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    print("wifi RF NFET-array coupon physical: "
          f"DRC={result['drc_error_count']} LVS={result['lvs_unique']} "
          f"devices={device_count} PEX={result['pex_resistor_count']}R/"
