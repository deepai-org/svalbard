#!/usr/bin/env python3
"""Render the compact physical WRITE-to-capture-clock bridge."""
...
layout = pya.Layout()
layout.read("/work/capture_clock_bridge.gds")
top = layout.top_cell()
bbox = top.bbox()
...
for order, layer_index in enumerate(layout.layer_indices()):
    ...
    draw, color = ImageDraw.Draw(overlay), COLORS[order % len(COLORS)]
    for shape in top.shapes(layer_index).each():
        if shape.is_box():
            ...
            draw.polygon([pixel(point) for point in shape.polygon.each_point_hull()], fill=color, outline=color[:3] + (225,))
        elif shape.is_text():
            draw.text(pixel(shape.text.trans.disp), shape.text.string, fill=(255, 255, 255, 255))
    canvas = Image.alpha_composite(canvas, overlay)
ImageDraw.Draw(canvas).text((MARGIN, 15), f"GF180 local capture-clock bridge | {bbox.width() * layout.dbu:.1f} x {bbox.height() * layout.dbu:.1f} um", fill=(235, 239, 245, 255))
canvas.convert("RGB").save("/work/capture-clock-bridge-layout.png")
