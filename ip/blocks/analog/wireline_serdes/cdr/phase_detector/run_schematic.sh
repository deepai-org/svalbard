...
source_dir="$repo_root/ip/blocks/analog/wireline_serdes/cdr/phase_detector"
...
for command_name in docker flock timeout mktemp awk df; do
  command -v "$command_name" >/dev/null 2>&1 || { printf 'cdr-phase-detector: missing host command: %s\n' "$command_name" >&2; exit 2; }
done
for checked_path in "$repo_root" "$(docker info --format '{{.DockerRootDir}}')"; do
  (( $(free_kib "$checked_path") >= minimum_free_kib )) || { printf 'cdr-phase-detector: less than 100 GiB free at %s\n' "$checked_path" >&2; exit 2; }
done
available_memory_kib="$(awk '/^MemAvailable:/ { print $2 }' /proc/meminfo)"
(( available_memory_kib >= minimum_available_memory_kib )) || { printf 'cdr-phase-detector: less than 8 GiB available memory\n' >&2; exit 2; }
...
[[ "$actual_image_id" == "$expected_image_id" && "$actual_architecture" == "arm64" ]] || { printf 'cdr-phase-detector: pinned ARM64 analog image is missing or mismatched\n' >&2; exit 2; }

mkdir -p "$scratch_root"
run_dir="$(mktemp -d "$scratch_root/cdr-phase-detector.XXXXXXXX")"
chmod 700 "$run_dir"
completed=0
finish() {
  if (( completed == 1 )); then find "$run_dir" -depth -delete; else printf 'cdr-phase-detector: failed outputs retained at %s\n' "$run_dir" >&2; fi
}
trap finish EXIT
exec 9>"$scratch_root/heavy-job.lock"
flock -n 9 || { printf 'cdr-phase-detector: another bounded heavy job is running\n' >&2; exit 2; }

timeout 45m docker run --rm --cpus=4 --memory=4g --memory-swap=4g --pids-limit=256 \
