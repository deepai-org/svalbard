#!/usr/bin/env python3
"""Render the generated CDR sampler GDS as a reviewable physical-layout PNG."""

import pya
from PIL import Image, ImageDraw, ImageFont

WIDTH, HEIGHT, MARGIN = 1800, 1100, 65
COLORS = [
...
layout = pya.Layout()
layout.read("/work/cdr_sampler.gds")
top = layout.top_cell()
bbox = top.bbox()
...
for order, layer_index in enumerate(layout.layer_indices()):
    ...
    for shape in top.shapes(layer_index).each():
        if shape.is_box():
            ...
            draw.polygon(
                [pixel(point) for point in shape.polygon.each_point_hull()],
...
annotate("EVEN latch", -24, 14, (80, 227, 194, 255))
annotate("ODD latch", 24, 14, (80, 227, 194, 255))
annotate("clock steering", -24, -7, (245, 166, 35, 255))
annotate("clock steering", 24, -7, (245, 166, 35, 255))
annotate("programmable tails", 0, -19, (189, 103, 217, 255))
annotate("contacted guard ring", 0, 27.6, (255, 92, 92, 255))

canvas.convert("RGB").save("/work/cdr_sampler-layout.png", quality=94)
