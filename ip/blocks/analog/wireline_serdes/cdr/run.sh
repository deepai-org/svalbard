...
for command_name in docker flock timeout mktemp awk df; do
  command -v "$command_name" >/dev/null 2>&1 || { printf 'cdr-sampler: missing host command: %s\n' "$command_name" >&2; exit 2; }
done
for checked_path in "$repo_root" "$(docker info --format '{{.DockerRootDir}}')"; do
  (( $(free_kib "$checked_path") >= minimum_free_kib )) || { printf 'cdr-sampler: less than 100 GiB free at %s\n' "$checked_path" >&2; exit 2; }
done
available_memory_kib="$(awk '/^MemAvailable:/ { print $2 }' /proc/meminfo)"
(( available_memory_kib >= minimum_available_memory_kib )) || { printf 'cdr-sampler: less than 8 GiB available memory\n' >&2; exit 2; }
...
[[ "$actual_image_id" == "$expected_image_id" && "$actual_architecture" == "arm64" ]] || { printf 'cdr-sampler: pinned ARM64 analog image is missing or mismatched\n' >&2; exit 2; }

mkdir -p "$scratch_root"
run_dir="$(mktemp -d "$scratch_root/cdr-sampler.XXXXXXXX")"
chmod 700 "$run_dir"
completed=0
finish() {
  if (( completed == 1 )); then find "$run_dir" -depth -delete; else printf 'cdr-sampler: failed outputs retained at %s\n' "$run_dir" >&2; fi
}
trap finish EXIT
exec 9>"$scratch_root/heavy-job.lock"
flock -n 9 || { printf 'cdr-sampler: another bounded heavy job is running\n' >&2; exit 2; }

timeout --kill-after=30s 300m docker run --rm --cpus=2 --memory=4g --memory-swap=4g --pids-limit=256 \
...
cp "$run_dir/result.json" "$scratch_root/cdr-sampler-last.json"
cp "$run_dir/cdr_sampler-layout.png" "$scratch_root/cdr-sampler-layout-last.png"
chmod 600 "$scratch_root/cdr-sampler-last.json" "$scratch_root/cdr-sampler-layout-last.png"
completed=1
printf 'cdr-sampler: PASS\ncdr-sampler: result %s\ncdr-sampler: render %s\n' \
  "$scratch_root/cdr-sampler-last.json" "$scratch_root/cdr-sampler-layout-last.png"
