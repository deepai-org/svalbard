#!/usr/bin/env python3
"""Render the clock level converter GDS for review."""

import pya
from PIL import Image, ImageDraw


WIDTH, HEIGHT, MARGIN = 1600, 1100, 50
...
layout = pya.Layout()
layout.read("/work/clock_level_converter.gds")
top = layout.top_cell()
bbox = top.bbox()
scale = min((WIDTH - 2 * MARGIN) / bbox.width(),
...
for order, layer_index in enumerate(layout.layer_indices()):
    ...
    for shape in top.shapes(layer_index).each():
        if shape.is_box():
            ...
            draw.polygon([pixel(point)
                          for point in shape.polygon.each_point_hull()],
...
ImageDraw.Draw(canvas).text(
    (MARGIN, 15),
    f"GF180 clock_level_converter | "
    f"{bbox.width() * layout.dbu:.1f} x {bbox.height() * layout.dbu:.1f} um",
    fill=(235, 239, 245, 255))
canvas.convert("RGB").save("/work/clock_level_converter-layout.png")
