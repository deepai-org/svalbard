...
python3 /src/clock_pulse_hclk_window_probe/test_event_capture_integrated_physical.py
python3 /src/clock_pulse_hclk_window_probe/compile_event_capture_integrated_physical_source.py \
...
python3 /src/clock_pulse_hclk_window_probe/summarize_event_capture_integrated_physical.py
rc=0
python3 /src/clock_pulse_hclk_window_probe/run_event_capture_schematic.py \
  ...
  --control-ids sense0_interval0_epoch1 sense0_interval1_epoch1 \
    sense1_interval0_epoch1 sense1_interval1_epoch1 \
  --work /work/pex-cases \
  --output /work/pex-result.json || rc=$?
if [[ "$rc" -ne 0 && "$rc" -ne 1 ]]; then
  exit "$rc"
fi
python3 /src/clock_pulse_hclk_window_probe/run_event_capture_integrated_pex_probe.py \
  --pex /work/retimed_event_capture_bridge.pex.spice \
  --capture /src/lane/capture_2p5_fast_deserializer.pex.spice \
  --work /work/pex-probe-cases --output /work/pex-probe-result.json
