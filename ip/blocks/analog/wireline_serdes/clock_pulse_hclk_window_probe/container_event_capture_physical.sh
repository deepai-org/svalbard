...
python3 /src/clock_pulse_hclk_window_probe/test_event_capture_physical_source.py
...
sak-lvs.sh -m -w /work/lvs \
  -s /work/retimed_event_capture_bridge.spice \
  -l /work/retimed_event_capture_bridge.mag \
  -c retimed_event_capture_bridge > /work/lvs-stage.log 2>&1
...
python3 /src/clock_pulse_hclk_window_probe/summarize_event_capture_physical.py
pex_rc=0
python3 /src/clock_pulse_hclk_window_probe/run_event_capture_schematic.py \
  ...
  --internal-probes \
  --work /work/event-capture-pex-cases \
  --output /work/retimed-event-capture-pex-result.json || pex_rc=$?
# Electrical rejection is retained evidence. Missing models, incomplete
# measurements, or tool failures use a different exit status and remain fatal.
if [[ "$pex_rc" -ne 0 && "$pex_rc" -ne 1 ]]; then
  exit "$pex_rc"
fi
