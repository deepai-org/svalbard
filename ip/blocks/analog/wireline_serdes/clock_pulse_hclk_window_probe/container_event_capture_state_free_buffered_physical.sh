#!/usr/bin/env bash
set -euo pipefail
python3 /src/clock_pulse_hclk_window_probe/test_event_capture_state_free_buffered_physical.py
python3 /src/clock_pulse_hclk_window_probe/compile_event_capture_state_free_buffered_physical_source.py \
...
python3 /src/clock_pulse_hclk_window_probe/summarize_event_capture_state_free_buffered_physical.py
rc=0
python3 /src/clock_pulse_hclk_window_probe/run_event_lane_composition.py \
  ...
  --event-physical /work/retimed-event-capture-buffered-physical.json \
  --event-schematic /work/retimed_event_capture_bridge.spice \
  --event-source-revision retimed_joint_long_6_3_capture_owned_start_v3_physical_lane_interface_v4 \
