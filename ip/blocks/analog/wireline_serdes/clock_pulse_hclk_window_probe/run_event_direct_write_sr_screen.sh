...
serdes=$(cd "$here/.." && pwd)
repo=$(cd "$here/../../../../.." && pwd)
work=$(mktemp -d "$repo/scratch/pcie-event-direct-write-sr-screen.XXXXXXXX")
trap 'rm -rf "$work"' EXIT
source_hash=$(sha256sum "$here/compile_event_capture_direct_write_sr_source.py" | cut -d' ' -f1)
timeout --kill-after=30s 20m docker run --rm --platform linux/arm64 \
  ...
  -lc /src/clock_pulse_hclk_window_probe/container_event_direct_write_sr_screen.sh || status=$?
status=${status:-0}
cp "$work/direct-write-sr-screen.json" "$repo/scratch/pcie-event-direct-write-sr-screen-last.json"
echo "pcie-event-direct-write-sr-screen: output $repo/scratch/pcie-event-direct-write-sr-screen-last.json"
exit "$status"
