...
class IntegratedEventStateTest(unittest.TestCase):
    ...
    def test_outputs_split_after_shared_local_predriver(self) -> None:
        source = integrated.compile_source()
        self.assertIn("XLS3 SIB SDRV", source)
        self.assertIn("XSB2 SDRV SSEL SENSE", source)
        self.assertIn("XLB2 LSTATE BOOST", source)
        self.assertIn("XLC0 ESTATE LCB VDD VSS cp_inv WP=4u WN=2u MP=4 MN=4", source)
        self.assertIn("XLC1 LCB LSTATE VDD VSS cp_inv WP=6u WN=3u MP=6 MN=6", source)
