#!/usr/bin/env bash
set -euo pipefail
/src/distributed_clock_fanout/container_physical.sh
python3 /src/distributed_clock_fanout/combine_pex.py \
  --sampler /work/sampler.pex.spice --capture /work/capture.pex.spice \
  --leaf-physical /work/physical_result.json \
  --output /work/distributed_fanout.pex.spice \
  --physical-output /work/distributed_fanout_physical.json
rc=0
python3 /src/clock_pulse_hclk_window_probe/run_event_lane_composition.py \
  ...
  --clock-fanout-pex /work/distributed_fanout.pex.spice \
  --clock-fanout-physical /work/distributed_fanout_physical.json \
