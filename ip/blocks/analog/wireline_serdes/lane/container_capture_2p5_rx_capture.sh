...
run_case() {
  ...
  python3 /src/lane/run_capture_stress_case.py "${common_args[@]}" \
    --case-id "$name" "$@" --work "/work/capture-2p5-rxcap-${name}" \
    --output "/work/capture-2p5-rxcap-${name}.json"
}
...
python3 /src/lane/merge_capture_2p5_calibrated.py \
  --case /work/capture-2p5-rxcap-tt.json \
  --case /work/capture-2p5-rxcap-ff_cold.json \
  --case /work/capture-2p5-rxcap-ff_hot.json \
  --case /work/capture-2p5-rxcap-ss_hot.json \
  --case /work/capture-2p5-rxcap-ss_passive.json \
  --claim routed_rx_capture_parent_extracted_2p5_gts_combined_stress_pvt \
  --physical-composition routed_termination_rx_spine_dual_converter_capture_parent \
  --output /work/capture-2p5-rxcap.json

python3 /src/lane/check_rx_capture_evidence.py /work
