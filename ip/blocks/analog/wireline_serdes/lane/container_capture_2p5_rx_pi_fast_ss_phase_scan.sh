...
common_args=(
  ...
  --sampler-phase 135 --rx-window-start-ps 250 --offset-ps 600
)
...
for phase in 45 225 270 315; do
  latency=1
  if ((phase < 135 || phase >= 315)); then
    latency=0
  fi
  output="/work/ss-phase-${phase}.json"
  cases+=("$output")
  python3 /src/lane/run_capture_stress_case.py "${common_args[@]}" \
    --case-id "ss_phase_${phase}" --pi-input-phase-deg "$phase" \
    --latency-ui "$latency" --sampler-latency-ui "$latency" \
    --frontend-latency-ui "$latency" \
    --frontend-write-latency-ui "$latency" \
    --capture-latency-ui "$latency" \
    --work "/work/ss-phase-${phase}" --output "$output" &
  pids+=("$!")
  if ((${#pids[@]} == 2)); then
    wait "${pids[@]}"
    pids=()
...
python3 /src/lane/summarize_capture_scan.py "${merge_args[@]}" \
  --output /work/ss-phase-scan.json
