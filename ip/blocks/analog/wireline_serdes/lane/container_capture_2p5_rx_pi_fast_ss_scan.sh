...
pids=()
cases=()
for skew in -150 -50 50 150 250 350 450; do
  tag="$(printf '%+04d' "$skew" | tr '+-' 'pm')"
  output="/work/ss-even-skew-${tag}.json"
  cases+=("$output")
  python3 /src/lane/run_capture_stress_case.py "${common_args[@]}" \
    --case-id "ss_even_skew_${tag}" --even-frontend-skew-ps "$skew" \
    --work "/work/ss-even-skew-${tag}" --output "$output" &
  pids+=("$!")
  if ((${#pids[@]} == 2)); then
    wait "${pids[@]}"
    pids=()
...
python3 /src/lane/summarize_capture_scan.py "${merge_args[@]}" \
  --output /work/ss-even-skew-scan.json
