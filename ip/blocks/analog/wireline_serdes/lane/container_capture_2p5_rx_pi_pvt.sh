...
run_case() {
  ...
  python3 /src/lane/run_capture_stress_case.py "${common_args[@]}" \
    --case-id "$name" "$@" --work "/work/capture-2p5-rx-pi-${name}" \
    --output "/work/capture-2p5-rx-pi-${name}.json"
}
...
run_case tt --tx-load-code 2 --tx-bias 1.5 --ac-initial-v 0.435 \
  ...
  --capture-delay-ps 550 --capture-output-delay-ps 1050 \
  --offset-ps 200 & p1=$!
...
python3 /src/lane/merge_capture_2p5_calibrated.py \
  ...
  --case /work/capture-2p5-rx-pi-ff_cold.json \
  --case /work/capture-2p5-rx-pi-ff_hot.json \
  --case /work/capture-2p5-rx-pi-ss_hot.json \
  --case /work/capture-2p5-rx-pi-ss_passive.json \
  --claim routed_pi_rx_capture_parent_extracted_2p5_gts_combined_stress_pvt \
  --physical-composition routed_phase_interpolator_termination_rx_dual_capture_parent \
  --output /work/capture-2p5-rx-pi.json
