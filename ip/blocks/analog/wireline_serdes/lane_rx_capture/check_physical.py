...
result = {
    "schema_version": 1,
    "claim": "routed_termination_rx_spine_dual_converter_capture_parent",
    ...
    "schematic_source_sha256": sha256_file(args.source / "lane_rx_capture.spice"),
    "checker_source_sha256": sha256_file(Path(__file__)),
}
passed = (
    result["drc_error_count"] == 0
    and result["lvs_unique"]
    and result["pex_resistor_count"] >= 7_000
    and result["pex_capacitor_count"] >= 4_500
    and result["layout_image_bytes"] >= 20_000
)
result["result"] = "pass" if passed else "fail"
args.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
print(f"lane RX capture: DRC={result['drc_error_count']} "
