...
result = {
    "schema_version": 1,
    "claim": "routed_termination_rx_dual_regenerative_sampler_frontend",
    ...
    "schematic_source_sha256": sha256_file(
        args.source / "lane_rx_regenerative_frontend.spice"),
    "checker_source_sha256": sha256_file(Path(__file__)),
}
passed = (
    result["drc_error_count"] == 0
    and result["lvs_unique"]
    and result["pex_resistor_count"] >= 4_000
    and result["pex_capacitor_count"] >= 2_400
    and result["layout_image_bytes"] >= 15_000
)
result["result"] = "pass" if passed else "fail"
args.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
print(f"regenerative RX front end: DRC={result['drc_error_count']} "
