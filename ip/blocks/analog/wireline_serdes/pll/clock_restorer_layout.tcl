...
# Compact matched CML clock-restoration stage for the VCO/divider boundary.
...
select top cell
flatten cml_clock_restorer
load cml_clock_restorer
units microns
...
# Adjacent 7.5 um loads and compact drain/output trunks.
foreach x {-4 4} {
    foreach y {12.25 19.75} { stack_to $x $y 3 }
    paint_rect metal3 [expr {$x-0.45}] 4.30 [expr {$x+0.45}] 12.25
    paint_rect metal3 [expr {$x-0.45}] 19.75 [expr {$x+0.45}] 22.0
...
save /work/cml_clock_restorer
gds write /work/cml_clock_restorer.gds
quit -noprompt
