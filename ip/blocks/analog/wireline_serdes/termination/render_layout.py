#!/usr/bin/env python3
"""Render the generated termination GDS for visual review."""
...
layout = pya.Layout()
layout.read("/work/serdes_termination.gds")
top = layout.top_cell()
bbox = top.bbox()
...
ImageDraw.Draw(canvas).text((MARGIN, 15),
    f"GF180 serdes_termination | {bbox.width() * layout.dbu:.1f} x "
    f"{bbox.height() * layout.dbu:.1f} um", fill=(235, 239, 245, 255))
canvas.convert("RGB").save("/work/serdes_termination-layout.png")
