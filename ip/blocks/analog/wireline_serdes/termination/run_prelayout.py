#!/usr/bin/env python3
"""Characterize programmable differential termination before layout."""
...
def main() -> None:
                ...
                for temp in (-40, 27, 125):
                    for cm_fraction in (0.35, 0.50, 0.65):
                        for enabled in range(8):
                            ...
                            cases.append({"id": case_id, "mos_corner": mos,
                                          "res_corner": resistor, "supply_v": vdd,
                                          "temperature_c": temp, "common_mode_fraction": cm_fraction,
                                          "enabled_branches": enabled, "observed": observed,
                                          "result": "pass" if complete else "fail"})
    complete = sum(case["result"] == "pass" for case in cases)
    ...
    passed = complete == len(cases) and calibrated_count == len(calibrated)
    result = {"schema_version": 1, "result": "pass" if passed else "fail",
              "case_count": len(cases), "complete_case_count": complete,
    ...
    print(f"termination prelayout: {complete}/{len(cases)} complete; "
