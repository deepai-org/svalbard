...
## Research direction: single-chip programmable wired-and-wireless transceiver platform
...
- **RF/wired sharing:** use specialized electrical front ends feeding a related family of local sample/weight/sum/decision tiles. Pool digital services and banked capture/playback memory with per-engine throughput guarantees; share calibration control and master references with local trims, filtering, and buffers. Give wired diagnostics selected buffered access to the RF/baseband converters without placing shared converters on the mandatory line-rate data path. Expose bounded local analog connections for repurposing idle resources. Distinguish circuit-design reuse from exclusive physical-resource ownership, reject conflicting configurations, and retain independent RF synthesis and wired clock recovery. Account for monitor loading, arbitration, reference coupling, and isolation within the existing area/power/noise budgets.
- **Die and interface:** fit one wafer.space 1×1 slot (approximately 3.93 × 5.12 mm) and **50 total external connections**, provisionally 36 signals plus 14 supply/ground. The [companion architecture and pin plan](docs/roadmap/programmable-transceiver-pin-plan.md) allocates two 10-bit source-synchronous GPIO buses with separate forwarded clocks, dedicated four-wire SPI, one external reference input, reset, wired TX/RX pairs, and RF TX/RX pairs. Use a custom mixed-signal pad ring and qualified assembly; retain a 12.92 mm² planning core ceiling until actual placement proves fit.
- **Host capability:** support ordinary FPGA GPIO without multi-gigabit transceivers, using a new explicitly specified `svalbard_phy_gpio_v1` transport. The ambitious 156.25 MHz DDR profile provides 3.125 Gb/s raw per direction and about 2.881 Gb/s after frame headers and integrity overhead. Qualify actual pad voltages, FPGA clock/DDR resources, package timing, and simultaneous switching. Generic compatibility does not promise maximum speed on every FPGA. MCU SPI supports configuration, finite captures/playback, and low-rate operation, not full-rate software PHY processing.
- **Examples and scope:** target configurable 1.25 GBd / 2.5 GT/s NRZ, a 1000BASE-X system with external FPGA PCS/MAC, a PCIe Gen1 x1 endpoint system with a verified external soft controller and PHY adapter, and a 2.4 GHz RF configuration for Wi-Fi/custom SDR with external FPGA DSP/modem/MAC. These examples define a difficulty class, not a closed application list. Full HDMI, copper 1000BASE-T, higher PCIe generations, and terabit Ethernet are not promised by this allocation.
- **Integration and concurrency:** RF and wired payload operation are mutually exclusive; both capabilities remain required on the same chip. Encourage additional physical resource sharing, with stopped/isolate/reconfigure/settle/qualify transitions and explicit ownership. Historical simultaneous traffic is optional stress evidence. See the [exclusive-engine policy](projects/programmable_transceiver_platform/spec/exclusive-engine-policy.md). External reference oscillator, matching/filter networks, antenna, optional PA, and host FPGA remain board components.
- **Unknowns (user requirement):** challenge both adverse extremes of every material uncertainty and design the full companion to tolerate them. Record defensible lower/upper bounds, adverse combinations and interior resonances or non-monotonic behavior; use trims, isolation and adequate margins with verified range. Chosen stress values are not proven bounds. If an unknown is not bounded, keep its affected claim open and obtain evidence or change the design to make it insensitive; do not invent a convenient bound, assume typical behavior, or silently reduce the full capability target. Numerical error must be challenged in both directions: apparent stability and apparent instability can both be misleading.
- **Operating envelope (user requirement):** narrow temperature and supply-voltage ranges are acceptable for this chip, including peak performance around a specific controlled operating point. Broad commercial/industrial temperature or supply-tolerance coverage is not a requirement. Select and document numerical junction-temperature and per-rail voltage windows from evidence; none are fixed yet. Include self-heating, rail ripple/droop, measurement/control tolerances, process variation and mismatch in closure within that envelope. This scope applies to this companion only and does not waive device reliability limits or imply nominal-only simulation proves silicon operation.
- **First fabrication — the full companion:** fabricate the complete declared analog/PHY platform with integrated timing, converters, transport, generic helpers, and diagnostic access. The companion itself is the full product; on-chip Wi-Fi modem/MAC or PCIe endpoint logic is not required. Do not fabricate a preliminary single-island feasibility chip or substitute a reduced demonstration vehicle. Freeze actual supported configurations, power, area, bandwidth, and package budgets at Gate G1.
- **Feasibility before fabrication; diagnostics on the full die:** establish extracted RF/wired and GPIO-link performance, mismatch/noise, control latency, package/substrate effects, and FPGA interoperability before tapeout. Keep SPI independent of high-speed clocks; include raw paths, PRBS, loopbacks, capture/playback, monitors, per-engine reset, and calibration. After fabrication measure RF quality, serial BER/jitter tolerance, GPIO eye/timing margins, end-to-end interoperability, and declared coexistence modes independently.

Apply O1–O4 and G0–G5 to this direction. Use pre-silicon evidence and already-available characterization to freeze the full companion; do not require a separately fabricated feasibility island. If closure fails, continue design work rather than silently substituting a smaller experiment. Record unsupported models, performance, and host combinations explicitly; measurements of the full chip guide claims and any respin.
