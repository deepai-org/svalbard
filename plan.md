...
1. **`wireline_serdes`:** static-CML TX/RX, serializer/deserializer, phase detection and clock recovery, PRBS, eye scan, loopbacks, and shared 8b/10b primitives. It operates at 1.25 GBd for 1000BASE-X and 2.5 GT/s for PCIe Gen1, using half-rate or quarter-rate internal clocks. `ethernet_mac_phy` and `pcie_gen1_endpoint` add their protocol-specific PCS, training, termination, receiver-detect, and electrical controls around the macro.
2. **`radio_baseband`:** ADC/DAC, voltage and current references, AGC support, DC and I/Q correction, sample capture/playback, numerically controlled oscillators, and multirate digital filtering. `wifi_nbiot_radio` shares this infrastructure between WiFi and NB-IoT but uses separate LNA, mixer, synthesizer band, PA, matching, and RF signal paths. Do not compromise the RF front ends into a nominally universal circuit.

These names describe integration assemblies, not indivisible release units. Release the smallest independently verifiable physical unit—for example `serdes_tx`, `serdes_rx`, `cdr`, `pll`, `adc`, `dac`, and `reference`—plus a separately versioned assembly manifest. This limits the blast radius of a change and avoids forcing unrelated consumers to accept an entire floorplanned tile. The Ethernet 10/100 required path does not depend on the high-speed SerDes merely because 1000BASE-X is a stretch target; a stretch feature never becomes a hidden dependency of the required v1 path.
...
5. Start bounded domain workstreams after the smoke loop:
   ...
   - **Shared physical-block track:** develop the SerDes, 10/100 line PHY, qualified pads/ESD, references, clocking, converters, RF blocks, calibration controller, diverse fallback variants, raw access, and the dedicated high-risk macro-matrix die through Gate G3. Harden and release each independently verifiable macro rather than one monolith.
