...
for code in (64,127,129,192):
 ...
 for bit in range(8):
  ...
  d=d.replace('.control',f'XDRV{bit} D{bit} B{bit} B{bit}B VDRV 0 pt_adc_code_driver WEIGHT={2**bit}\n.control')
 d=d.replace('.control','.include /screen/adc/code_driver.spice\nVDRV VDRV 0 3.3\n.control')
...
sources=json.loads((B/'result.json').read_text())['source_sha256'];sources['/screen/adc/code_driver.spice']=hashlib.sha256(Path('/screen/adc/code_driver.spice').read_bytes()).hexdigest()
r=dict(status='actual_cdac_gate_driver_screen_not_sar',cases=rows,source_sha256=sources,limitations=['Matched TT 3.3V 27C, ideal capacitors and reference fixtures unchanged.', 'Actual three-inverter driver with unequal true/complement paths, no enforced nonoverlap.', 'Logic commands remain ideal and drive first-stage input capacitance; SAR controller and its timing/power absent.', 'Driver supply ideal and separately metered; no PDN coupling, physical parasitics or mismatch.', 'Only four scripted code transitions and return; not ADC transfer/ENOB qualification.'])
