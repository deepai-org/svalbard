...
def main(small=False):
 circuit='code_driver_small.spice' if small else 'code_driver.spice'
 subckt='pt_adc_code_small_driver' if small else 'pt_adc_code_driver'
 status='small_cdac_gate_driver_screen_not_sar' if small else 'actual_cdac_gate_driver_screen_not_sar'
 ...
 for code in (64,127,129,192):
  ...
  for bit in range(8):
   ...
   d=d.replace('.control',f'XDRV{bit} D{bit} B{bit} B{bit}B VDRV 0 {subckt} WEIGHT={2**bit}\n.control')
  d=d.replace('.control',f'.include /screen/adc/{circuit}\nVDRV VDRV 0 3.3\n.control')
 ...
 sources=json.loads((B/'result.json').read_text())['source_sha256'];sources[f'/screen/adc/{circuit}']=hashlib.sha256(Path(f'/screen/adc/{circuit}').read_bytes()).hexdigest()
 r=dict(status=status,cases=rows,source_sha256=sources,limitations=['Matched TT 3.3V 27C, ideal capacitors and reference fixtures unchanged.', 'Actual three-inverter driver with unequal true/complement paths, no enforced nonoverlap.', 'Logic commands remain ideal and drive first-stage input capacitance; SAR controller and its timing/power absent.', 'Driver supply ideal and separately metered; no PDN coupling, physical parasitics or mismatch.', 'Only four scripted code transitions and return; not ADC transfer/ENOB qualification.'])
