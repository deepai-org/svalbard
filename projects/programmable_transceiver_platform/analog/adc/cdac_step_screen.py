...
def main(scaled=False):
 circuit='cdac8_scaled' if scaled else 'cdac8'
 status='scaled_switch_cdac_step_screen_not_adc' if scaled else 'transistor_switched_cdac_step_screen_not_adc'
 O=Path('/work');rows=[]
...
.include /screen/adc/{circuit}.spice
...
XD HP HN VH VL {ports} VDD 0 pt_{circuit}
...
'''
 ...
 r=dict(status=status,cases=rows,source_sha256={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in (Path('/screen/adc/comparator.spice'),Path(f'/screen/adc/{circuit}.spice'),Path('/wifi/rf_if_transmission_gate/rf_if_transmission_gate.spice'))},limitations=['Nominal matched TT 3.3V 27C; ideal capacitors 20fF unit, 5.12pF per side.', 'Ideal reference sources behind 10ohm and 10pF local capacitance; not implemented reference drivers.', 'Ideal external code/complement and clock waveforms; no SAR controller or output latch.', 'Two scripted decisions with code return, not a complete conversion or transfer curve.', 'No mismatch, capacitor nonlinearity, noise, physical parasitics, previous-sample acquisition or ENOB qualification.'])
