"""Actual dualADC with hybrid high input stage and doubled output PMOS."""
...
m=dict(parent_deck_sha256=sha(B/'frames.spice'),deck_sha256_before=sha(p),source_sha256_before=before,changes=changes,added_subcircuit=cell,scope='Five high-reference input/tail/mirror devices use NMOS-input topology; PMOS output multiplicity doubled; all ADC circuitry/history retained.',evaluation='Compare both gate transition directions, charge/peaks, all reference decision errors, held voltages, codes and driver supply energy. No automatic promotion from a reduced peak.')
