...
CP QP 0 50f
CN QN 0 50f
XCTL QN UPDATE START RN {bits} DONE VLOG 0 pt_sar8_control
{drivers}
.control
set wr_singlescale
set wr_vecnames
set numdgt=15
tran 10p 274n 0 10p
wrdata /work/{name}.dat v(HP) v(HN) v(QP) v(QN) v(CLK) v(UPDATE) v(DONE) {' '.join('v(D'+str(i)+')' for i in range(8))} {' '.join('v(XCTL.T'+str(i)+')' for i in range(8))} i(VLOG) i(VDRV)
.endc
.end
'''
