...
for sign in (-1,1):
 ...
 extra='.include /screen/adc/sar_track_mask.spice\nXTRACK '+ ' '.join(f'D{i}' for i in range(8))+' MASKB '+' '.join(f'SD{i}' for i in range(8))+' VLOG 0 pt_sar_track_mask\n'
 mask=[(0,0)]
...
r=dict(status='early_midcode_external_phase_frames_not_qualified_adc',cases=rows,source_sha256=json.loads((B/'result.json').read_text())['source_sha256'],limitations=['External reset/start/clock/sample waveforms; no actual phase generator or output register.', 'Three frames in each polarity sequence, ideal capacitors/bias references and separate supplies.', 'No continuous transfer, ENOB, noise/mismatch/process or physical loading qualification.', 'Code must be observed before next reset; host capture timing unimplemented.'])
r['source_sha256']['/screen/adc/sar_track_mask.spice']=hashlib.sha256(Path('/screen/adc/sar_track_mask.spice').read_bytes()).hexdigest()
