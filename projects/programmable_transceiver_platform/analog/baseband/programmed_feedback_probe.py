...
.include /screen/bb_filter_programmable.spice
.temp 27
VDD VDD 0 3.3
VB BIAS 0 2.25
XDUT IP IN OP ON BIAS VDD 0 SEL SELB pt_bb_filter_programmable C=20p
'''
paths=set()
...
probes += ['v(XDUT.FBP)','v(XDUT.FBN)']
(O/'manifest.json').write_text(json.dumps(dict(source_sha256_before=before,common_mode_v=[1.177],source_ohm_per_leg=[1000],feedback_ohm=[20000,40000],op_probes=probes,scope='TT27C3.3V external bias2.25V; original RFB20k/C20p; source/load scenarios not fab bounds'),indent=2)+'\n')
rows=[]
for cm in (1.177,):
