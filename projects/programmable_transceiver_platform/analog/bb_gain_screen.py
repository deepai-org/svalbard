...
def main(pmos=False, *, entrypoint=None):
    biases=(2.25,2.35,2.45) if pmos else (.85,.95,1.05)
...
'''
    ...
    result=dict(status='dc_ac_primitive_screen_not_filter_qualification',supply_v=3.3,temperature_c=27,process='typical',
        ...
        source_sha256={str(p):digest(p) for p in (Path(__file__),SRC/circuit,*([] if entrypoint is None else [Path(entrypoint)]))},
