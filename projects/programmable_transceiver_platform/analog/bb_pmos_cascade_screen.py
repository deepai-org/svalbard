#!/usr/bin/env python3
"""Actual two-cell PMOS cascade with finite input loading, nominal AC/DC only."""
...
.include /screen/bb_pmos_gain.spice
.temp 27
VDD VDD 0 3.3
VB BIAS 0 {bias}
...
CP OP 0 {cap_pf}p
CN ON 0 {cap_pf}p
XDUT IP IN OP ON TAIL BIAS VDD pt_bb_pmos_gain
R2P FP 0 1k
R2N FN 0 1k
C2P FP 0 5p
C2N FN 0 5p
XSECOND OP ON FP FN TAIL2 BIAS VDD pt_bb_pmos_gain
.control
op
let outcm=(v(FP)+v(FN))/2
let firstcm=(v(OP)+v(ON))/2
let tail2dc=v(TAIL2)
let taildc=v(TAIL)
let supply=-i(VDD)
print outcm taildc supply firstcm tail2dc
ac dec 60 100k 1gig
let gain=mag(v(FP)-v(FN))
let phase=180/PI*cph(v(FP)-v(FN))
...
'''
    ...
    measures={}
    for key in ('outcm','taildc','supply','firstcm','tail2dc','gain_1m','gain_10m','gain_30m','gain_100m','phase_10m'):
...
result=dict(status='dc_ac_primitive_screen_not_filter_qualification',supply_v=3.3,temperature_c=27,process='typical',
    ...
    source_sha256={str(p):digest(p) for p in (Path(__file__),SRC/'bb_pmos_gain.spice')},
    ...
    limitations=['Ideal external gate bias and resistor/capacitor models.',
                 ...
                 'Two identical cells directly cascaded; small-signal check only, no filter or receiver integration.'])
