#!/usr/bin/env python3
"""PMOS-input differential-cell common-mode/amplitude transient diagnostic."""
...
result=dict(status='common_mode_swing_diagnostic_not_filter_qualification',supply_v=3.3,temperature_c=27,process='typical',
    external_tail_bias_v=2.25,output_cap_per_leg_f=5e-12,load_per_leg_ohm=1000,cases=cases,
    source_sha256={str(p):digest(p) for p in (Path(__file__),SRC/'rf_measure.py',SRC/'bb_pmos_gain.spice')},
