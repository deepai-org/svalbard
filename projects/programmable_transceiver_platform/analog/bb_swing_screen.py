...
def main(pmos=False, *, entrypoint=None):
    bias=2.25 if pmos else 1.05
    circuit='bb_pmos_gain.spice' if pmos else 'bb_diff_gain.spice'
    resistors='RP OP 0 1k\nRN ON 0 1k' if pmos else 'RP VDD OP 1k\nRN VDD ON 1k'
    device='XDUT IP IN OP ON TAIL BIAS VDD pt_bb_pmos_gain' if pmos else 'XDUT IP IN OP ON TAIL BIAS 0 pt_bb_diff_gain'
    cases=[]
...
'''
    ...
    result=dict(status='common_mode_swing_diagnostic_not_filter_qualification',supply_v=3.3,temperature_c=27,process='typical',
        external_tail_bias_v=bias,output_cap_per_leg_f=5e-12,load_per_leg_ohm=1000,cases=cases,
        source_sha256={str(p):digest(p) for p in (Path(__file__),SRC/'rf_measure.py',SRC/circuit,*([] if entrypoint is None else [Path(entrypoint)]))},
