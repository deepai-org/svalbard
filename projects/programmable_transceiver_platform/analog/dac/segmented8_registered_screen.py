...
def main(coverage=False):
    ...
    sources = [Path(p) for p in old['source_sha256_before']] + [Path('/screen/dac/segmented8_registered.spice'), Path('/screen/pll/pfd.spice')]
    before = {str(p): sha(p) for p in sources}
    rows = []
    extra = ['v(XD.CK)', 'i(VCLK)'] + [f'v(XD.RL{i})' for i in range(4)] + [f'v(XD.RH{k})' for k in range(1, 16)]
    manifest = dict(source_sha256_before=before, planned_cases=['c32_skew0.2', 'c128_skew-0.2', 'c128_skew0', 'c128_skew0.2'] if coverage else ['c32_skew-0.2', 'c32_skew0'], extra_vectors=extra, clock_fixture='PULSE first6ns,period25ns,width5ns,100ps edges; reset release2ns', scope='Fixed capture opportunity ~1ns after nominal command onset; not setup/hold qualification')
    (O / 'manifest.json').write_text(json.dumps(manifest, indent=2) + '\n')
    for name in manifest['planned_cases']:
        src = B / (name + '.spice')
        c = next((c for c in old['cases'] if c['name'] == name))
