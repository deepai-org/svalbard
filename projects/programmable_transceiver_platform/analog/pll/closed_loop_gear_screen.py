...
d=d.replace('.control', '.options method=gear maxord=2\n.control')
p=O/'closed.spice';p.write_text(d)
with (O/'closed.log').open('w') as log:subprocess.run(['ngspice','-b',str(p)],stdout=log,stderr=subprocess.STDOUT,check=True,timeout=14400)
a=np.loadtxt(O/'closed.dat',skiprows=1);assert a.shape[1]==18 and np.isfinite(a).all() and a[-1,0]>3200e-9
rows=[]
for lo,hi in ((100,200),(220,320),(500,600),(800,900),(1100,1200),(2000,2100),(3000,3100)):
...
r=dict(status='gear2_settling_loop_numerical_crosscheck_not_lock_qualification',windows=rows,artifacts_sha256={s:hashlib.sha256((O/('closed'+s)).read_bytes()).hexdigest() for s in ('.spice','.dat','.log')},limitations=['3201ns with precharged filter, seeded VCO and prebiased LNA; no cold startup.', 'Finite seeded nominal record, no qualified lock/acquisition/stability or phase-noise claim.', 'Actual divide128/PFD/pump/filter/VCO, but ideal external bias sources/reference/sampling clocks.', 'Filter values are preliminary; no PVT/mismatch or physical parasitics.', 'Gear2 integration differs from aborted original run; numerical damping requires explicit comparison and does not prove physical stability.'])
