"""Shift active PFD feedback pulse by +/-1ps; diagnostic only."""
import hashlib,json,subprocess
from pathlib import Path
O=Path('/work');B=Path('/baseline')
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
r=json.loads((B/'result.json').read_text());prior=next(c for c in r['cases'] if c['name']=='mixed');original=(B/'mixed.spice').read_text();assert sha(B/'mixed.spice')==prior['artifacts_sha256']['.spice']
...
for name,delay in (('early','99.999n'),('late','100.001n')):
 d=original.replace(old,old.replace('100n',delay)).replace('/work/mixed.dat',f'/work/{name}.dat')
...
(O/'result.json').write_text(json.dumps(dict(baseline_deck_sha256=sha(B/'mixed.spice'),source_sha256_before=before,source_sha256_after=after,cases=rows),indent=2)+'\n')
