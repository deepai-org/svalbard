...
'''
 ...
 rows.append(dict(feedback_delay_ns=shift,equivalent_high_time_s=area,up_minus_down_s=area[0]-area[1],reset_output_max_v=float(reset[:,1:3].max()),artifacts_sha256={s:hashlib.sha256((O/(name+s)).read_bytes()).hexdigest() for s in ('.spice','.dat','.log')}))
r=dict(status='nominal_transistor_pfd_phase_direction_screen_not_pll',cases=rows,source_sha256=hashlib.sha256(Path('/screen/pll/pfd.spice').read_bytes()).hexdigest(),limitations=['25MHz, TT 3.3V 27C, 50fF output loads; no charge pump.', 'No frequency acquisition, dead-zone sweep, mismatch, supply perturbation or extracted timing.'])
