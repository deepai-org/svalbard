...
original=B.read_text();assert 'XD1 XRX.CP XRX.CN ' in original
...
(O/'manifest.json').write_text(json.dumps(dict(source_sha256_before=before,max_step_ps=.5 if FINE else 2,baseline_deck_sha256=sha(B),pulse_nodes=['XRX.CP','XRX.CN'],pulse_times_ns=[20,20.001,20.011,20.012],amplitudes_a=[0,1e-4,-1e-4],scope='Deterministic differential output charge perturbation; no intrinsic noise or autonomous PLL claim'),indent=2)+'\n')
rows=[]
