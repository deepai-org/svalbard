...
r=dict(status='independent_regen_bias_lower_tuning_screen_not_pll',regen_bias_v=1.08,cases=rows,slopes_hz_per_v=[(b['frequency_hz']-a['frequency_hz'])/(b['control_v']-a['control_v']) for a,b in zip(rows,rows[1:])],limitations=['Nominal seeded/prebiased RF hierarchy; no acquisition, phase noise or process/mismatch.', 'Sampled tuning points do not establish global monotonicity or a loop-stability model.', 'Actual mixer/buffer load but ideal biases/sample clocks; actual seven-stage divider/interface/PFD load.'])
