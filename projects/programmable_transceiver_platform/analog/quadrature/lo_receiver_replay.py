...
s=(B/'replay.spice').read_text();scan(s,B);before={str(p):sha(p) for p in paths}
for n,h in before.items():
 if n in m['source_provenance']:assert m['source_provenance'][n]==h
p=O/'replay.spice';p.write_text(s);start=time.monotonic()
...
(O/'result.json').write_text(json.dumps(dict(returncode=code,timed_out=timeout,elapsed_s=time.monotonic()-start,sources_before=before,sources_after=after,artifacts_sha256={e:sha(O/('replay'+e)) for e in ('.spice','.log','.dat') if (O/('replay'+e)).exists()}),indent=2)+'\n');print(code,timeout)
