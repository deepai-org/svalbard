"""Actual tuned pair and rejected long mirrors, reservoir-loaded AC response."""
...
def main(candidate_variant='long_mirror', *, output2=False, entrypoint=None):
    rows = []
    for variant, root in [('hybrid', Path('/candidate'))] if output2 else [('baseline', Path('/base')), (candidate_variant, Path('/candidate'))]:
        ...
        pc = next((x for x in prior['cases'] if x['name'] == 'VH'))
        ...
        def scan(text, parentdir):
            for line in text.splitlines():
                ...
                if not m:
                    continue
                p = Path(m[1].strip(chr(34) + chr(39)))
                p = p if p.is_absolute() else parentdir / p
                if not p.is_file():
