...
def sampled_chain(base, f, O):
    rows = []
    waves = []
    for amplitude in (0, 0.001):
        name = f'a{amplitude:g}'
        d = base.replace('SIN(0 0.001 2.51g)', f'SIN(0 {amplitude} {f + 10000000.0})')
        ...
        assert a.shape[1] == 9 and np.isfinite(a).all() and (a[-1, 0] > 3e-07)
        ts = np.arange(1.24e-07, 3.01e-07, 2.5e-08)
        samples = np.interp(ts, a[:, 0], a[:, 4])
