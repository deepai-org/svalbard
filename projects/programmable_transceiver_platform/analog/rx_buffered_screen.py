...
cases=[]
for topology in ('ideal','buffered'):
    for amplitude in (0,.001):
        name=f'{topology}_{amplitude:g}'
        core=SRC/('rx_iq_split_core.spice' if topology=='ideal' else 'rx_iq_buffered_core.spice')
...
result=dict(status='simulation_completed_not_receiver_qualified',process='typical',temperature_c=27,supply_v=3.3,
    ...
    limitations=['Ideal input quadrature source, bias and passives; four transistor LO buffers in buffered case.',
