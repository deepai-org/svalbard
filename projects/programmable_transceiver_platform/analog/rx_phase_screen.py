#!/usr/bin/env python3
"""Stress quadrature phase in both directions with split LNA branches."""
import hashlib
import json
import math
...
cases=[]
for skew_ps in (-5,5):
    topology=f'skew{skew_ps:+}ps'
    for amplitude in (0,.001):
        name=f'{topology}_{amplitude:g}'
        core=SRC/'rx_iq_split_core.spice'
        ...
        for index,line in enumerate(lines):
            if line.startswith(('VQ ', 'VQB ')):
                fields=line.split()
                fields[5]=f'{float(fields[5])+skew_ps*1e-12:.16g}'
        ...
        assert all(len(p)==7 and all(map(math.isfinite,p)) for p in points)
        ...
        cases.append(dict(topology=topology,lo_q_skew_ps=skew_ps,input_peak_v=amplitude,
...
for skew_ps in (-5,5):
    ...
    i,q,gate=values
    summaries.append(dict(topology=topology,lo_q_skew_ps=skew_ps,
        ideal_phase_error_deg=skew_ps*1e-12*2.4e9*360,
        sideband_ratio_db=20*math.log10(abs(i-1j*q)/abs(i+1j*q)),
        ideal_phase_only_ratio_db=-20*math.log10(abs(math.tan(math.pi*2.4e9*skew_ps*1e-12))),
        gain_i_v_per_v=abs(i)/.001,gain_q_v_per_v=abs(q)/.001,
...
files=[Path(__file__),SRC/'rf_measure.py',SRC/'rx_branch_load_tb.spice.in',SRC/'rx_iq_core.spice',SRC/'rx_iq_split_core.spice',
...
result=dict(status='simulation_completed_not_receiver_qualified',process='typical',temperature_c=27,supply_v=3.3,
    ...
    stress_scope='Both Q and QB edges shifted together by +/-5 ps; duty/complement relationship preserved.',
    image='sha256:bd7a702bef0b85f5ebf67efca449f270fbeb185380ead204559fcd2457959305',
    source_sha256={str(p):digest(p) for p in files},
    limitations=['External ideal bias, LO and passives; schematic compact models.',
                 ...
                 'Chosen +/-5 ps stresses are not a characterized manufacturing or clock-error bound.',
                 'No noise/mismatch, PEX, input matching, calibration or broadband rejection qualification.',
                 'VDD current excludes external LO/bias-source power.',
