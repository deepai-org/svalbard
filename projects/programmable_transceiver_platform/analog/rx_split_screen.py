#!/usr/bin/env python3
"""Compare shared and duplicated LNA branches behind one finite source impedance."""
import hashlib
import json
import math
...
def main(*, buffered=False, entrypoint=None):
    topologies=('ideal','buffered') if buffered else ('shared','split')
    template=(SRC/'rx_branch_load_tb.spice.in').read_text()
    cases=[]
    for topology in topologies:
        for amplitude in (0,.001):
            name=f'{topology}_{amplitude:g}'
            core=SRC/({'shared':'rx_iq_core.spice', 'split':'rx_iq_split_core.spice',
                       'ideal':'rx_iq_split_core.spice', 'buffered':'rx_iq_buffered_core.spice'}[topology])
            ...
            if topology!='shared':
                ...
                # Extra diagnostic vector keeps both output formats identical.
                deck=deck.replace('.control','EOBS DRAIN_Q 0 MIX_RF 0 1\n.control')
            ...
            assert all(len(p)==(8 if buffered else 7) and all(map(math.isfinite,p)) for p in points)
            ...
            cases.append(dict(topology=topology,input_peak_v=amplitude,
                ...
                average_vdd_current_a=current,deck_sha256=digest(path)))
            if buffered:
                cases[-1]['lo_buffer_current_a']=-sum((b[0]-a[0])*(a[7]+b[7])/2 for a,b in zip(window,window[1:]))/(window[-1][0]-window[0][0])
    ...
    for topology in topologies:
        ...
        if buffered:
            summaries[-1].update(lo_buffer_current_a=signal['lo_buffer_current_a'],
                measured_total_current_a=signal['average_vdd_current_a']+signal['lo_buffer_current_a'])
    files=[Path(__file__),SRC/'rf_measure.py',SRC/'rx_branch_load_tb.spice.in',SRC/'rx_iq_split_core.spice',
           Path('/src/rf_lna/lna_cs_core.spice'),Path('/src/rf_switch_mixer/mixer.spice')]
    files += ([Path(entrypoint),SRC/'rx_iq_buffered_core.spice',SRC/'lo_buffer.spice'] if buffered else [SRC/'rx_iq_core.spice'])
    result=dict(status='simulation_completed_not_receiver_qualified',process='typical',temperature_c=27,supply_v=3.3,
        ...
        gain_change_db=20*math.log10(summaries[1]['gain_i_v_per_v']/summaries[0]['gain_i_v_per_v']),
        current_ratio=summaries[1]['average_vdd_current_a']/summaries[0]['average_vdd_current_a'],
        image='sha256:bd7a702bef0b85f5ebf67efca449f270fbeb185380ead204559fcd2457959305',
        source_sha256={str(p):digest(p) for p in files},
        limitations=[('Ideal input quadrature source, bias and passives; four transistor LO buffers in buffered case.' if buffered else 'External ideal bias, LO and passives; schematic compact models.'),
                     ...
                     ('Separate LNA and LO buffer currents; upstream oscillator/phase generator and bias power excluded.' if buffered else 'VDD current excludes external LO/bias-source power.'),
