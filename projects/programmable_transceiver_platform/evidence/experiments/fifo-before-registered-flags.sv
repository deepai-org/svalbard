...
module pt_async_fifo #(parameter W=10, A=6)(
 ...
 integer k; begin binary[A]=g[A]; for(k=A-1;k>=0;k=k-1) binary[k]=binary[k+1]^g[k]; end endfunction
...
module pt_sync_fifo #(parameter W=10,A=5)(input wire clk,rst_n,
 ...
 output wire full,empty,output reg[A:0]count,output reg overflow,underflow);
 reg[W-1:0]mem[0:(1<<A)-1];reg[A-1:0]wp,rp;
 assign full=count==(1<<A);assign empty=count==0;assign dout=mem[rp];
 wire put=push&&!full,take=pop&&!empty;
 always @(posedge clk or negedge rst_n) begin
  if(!rst_n)begin wp<=0;rp<=0;count<=0;overflow<=0;underflow<=0;end
  else begin
   if(put)begin mem[wp]<=din;wp<=wp+1'b1;end
   if(take)rp<=rp+1'b1;
   case({put,take})2'b10:count<=count+1'b1;2'b01:count<=count-1'b1;default:begin end endcase
