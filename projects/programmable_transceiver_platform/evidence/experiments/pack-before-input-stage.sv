...
module pt_pack(input wire clk,rst_n,mode8,input wire[23:0]sample,
 ...
 assign sample_ready=count<10;
 always @(posedge clk or negedge rst_n)begin
  if(!rst_n)begin bits<=0;count<=0;end
  else if(word_valid&&word_ready)begin bits<=bits>>10;count<=count-10;end
  else if(sample_valid&&sample_ready)begin
   bits<=bits|(({24'b0,sample}&(mode8?48'hffff:48'hffffff))<<count);count<=count+width;
  end
 end
endmodule
