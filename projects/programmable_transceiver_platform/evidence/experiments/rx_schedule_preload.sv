...
module pt_stream_rx(input wire clk,rst_n,mode8,input wire[9:0]in_word,
 ...
 // Preload the fixed schedule before validation; payload_active prevents any
 // effect until the complete header is accepted. The low bits directly
 // drive transfer qualification; no slot-counter decode sits on FIFO writes.
 reg[58:0]wire_slots,iq_slots;reg payload_active;
 ...
 assign data=in_word;
 assign wire_valid=rst_n&&!fault&&payload_active&&wire_slots[0]&&leftw!=0&&wire_ready;
 assign iq_valid=rst_n&&!fault&&payload_active&&iq_slots[0]&&leftq!=0&&iq_ready;
 always @(posedge clk or negedge rst_n)begin
  if(!rst_n)begin payload_active<=0;wire_slots<=0;iq_slots<=0;header<=0;pos<=0;expected_seq<=0;leftw<=0;leftq<=0;command_valid<=0;fault<=0;opcode<=0;argument<=0;end
  else begin
   command_valid<=0;
   if(!fault)begin
    case(pos)
     0:begin header[9:0]<=in_word;payload_active<=0;
      wire_slots<=slot_mask(mode8,1);iq_slots<=slot_mask(mode8,2);end
    ...
    endcase
    pos<=pos+1'b1;if(pos==63)begin expected_seq<=expected_seq+1'b1;payload_active<=0;end
   end
  end
 end
endmodule
