...
module pt_stream_tx(input wire clk,rst_n,mode8,
 ...
 // Snapshot counts at word zero; clamp at word one before payload starts.
 reg[6:0]snapshot_wire_count;reg[5:0]snapshot_iq_count;reg snapshot_mode;
 ...
 wire[1:0]own=owner(mode8,pos-6'd2);
 ...
 assign wire_pop=pos>=5&&own==1&&leftw!=0;
 assign iq_pop=pos>=5&&own==2&&leftq!=0;
 always @* begin
  out_word=0;
  case(pos)
 ...
 always @(posedge clk or negedge rst_n)begin
  if(!rst_n)begin snapshot_wire_count<=0;snapshot_iq_count<=0;snapshot_mode<=0;stage_valid<=0;stage_address<=0;stage_data<=0;active<=0;pos<=0;seq<=0;command[0]<=0;command[1]<=0;leftw<=0;leftq<=0;wc[0]<=0;wc[1]<=0;qc[0]<=0;qc[1]<=0;
   ...
   if(pos==0)begin
    ...
    snapshot_wire_count<=wire_count;snapshot_iq_count<=iq_count;snapshot_mode<=mode8;
   end
   if(pos==1)begin
    wc[!active]<=snapshot_wire_count>(snapshot_mode?7'd52:7'd33)?(snapshot_mode?6'd52:6'd33):snapshot_wire_count[5:0];
    qc[!active]<=snapshot_iq_count>(snapshot_mode?6'd7:6'd25)?(snapshot_mode?6'd7:6'd25):snapshot_iq_count;
    leftw<=snapshot_wire_count>(snapshot_mode?7'd52:7'd33)?(snapshot_mode?6'd52:6'd33):snapshot_wire_count[5:0];
    leftq<=snapshot_iq_count>(snapshot_mode?6'd7:6'd25)?(snapshot_mode?6'd7:6'd25):snapshot_iq_count;
   end
   if(pos>=5)begin
