...
module pt_stream_tx(input wire clk,rst_n,mode8,
 ...
 // Decode next word ownership; mode must remain frozen while armed.
 reg[1:0]own;
 ...
 always @(posedge clk or negedge rst_n)begin
  if(!rst_n)begin own<=0;snapshot_wire_count<=0;snapshot_iq_count<=0;snapshot_mode<=0;stage_valid<=0;stage_address<=0;stage_data<=0;active<=0;pos<=0;seq<=0;command[0]<=0;command[1]<=0;leftw<=0;leftq<=0;wc[0]<=0;wc[1]<=0;qc[0]<=0;qc[1]<=0;
