"""Local coupled RF-envelope/rail ODE for the exploratory driver supply law."""
import copy,math
import numpy as np
from scipy.integrate import solve_ivp
from rf_driver_supply import DriverSupplyLaw
from rf_switched_load import SwitchedLoad
from reference_buffer_power import account as reference_power

class CoupledDriver:
    def __init__(self,network=None,law=None,rail_r=100.,rail_c=100e-12,minimum_rail_v=2.5,detector=None,reference=None,reference_bias_a=.0001,reference_efficiency=.5):
        ...
        if reference is not None:
            ...
            reference_power(reference.voltage,1.,reference.r,self.rail_v,reference_bias_a,reference_efficiency)
    ...
    def advance(self,time,command,rtol=1e-8,atol=1e-11,max_step=math.inf):
        ...
        def rhs(t,y):
            ...
            if self.reference is not None:
                ...
                power=reference_power(y[ref_index],target,r.r,rail,self.reference_bias,self.reference_efficiency)
                result[8]-=power['dc_current_a']/self.c
                result=np.r_[result,power['output_current_a']/r.c]
            return result
