...
def main():
    ...
    for step in (4e-9,1e-9,.25e-9):
        ...
        for endpoint in np.linspace(10e-9,100e-9,10):
            ...
            part=advance_phase(load,float(endpoint),[(v*np.exp(p*start),p) for v,p in source],phase,step)
        ...
        rows.append(dict(step_s=step,**diag,monitor_power=load.detector.value))
    ...
    (P/'evidence/connected-rf-phase-forcing.json').write_text(json.dumps(report,indent=2)+'\n');print(rows)
