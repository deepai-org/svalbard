...
def main():
    c=SharedPhaseLoadedTxChip(watchdog_s=1e-3,adc_latency_s=50e-9,tx_relative_gain=True)
    ...
    report=dict(status='passed',phase_substeps=c.phase_steps,phase_events=c.phase_events,shared_adc_samples=c.tx_adc_samples,reference_charge_c=c.adc_reference.charge-charge,
        probe_powers=c.tx_cal.powers,correction=c.tx_cal.candidate,
        limitations=['Shared ADC and autonomous LO composition; host traffic and retuned quality not yet exercised.',
    ...
    (P/'evidence/connected-shared-phase-calibration.json').write_text(json.dumps(report,indent=2)+'\n');print(report)
