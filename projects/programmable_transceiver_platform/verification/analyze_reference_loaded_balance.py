...
e=P/'evidence/sar-reference-balance.json'
if not e.exists():
...
p=R/'scratch/transceiver-sar-reference-balance/baseline.dat';assert sha(p)==d['waveform_sha256']
    ...
    for j in range(8):
        ...
        for rail,node in [('xhigh','vh'),('xlow','vl')]:
            ...
            rows.append(dict(hold_ns=hold,bit=7-j,rail=rail,window_ns=.2,
                conduction_charge_c=qc,compensation_charge_c=qm,reservoir_charge_change_c=qr,cdac_outward_charge_c=qload,
                pre_cdac_residual_c=qc+qm-qr,
...
out=dict(reservoir_capacitance_f=cap,rows=rows,waveform_sha256=sha(p),probe_contract_sha256=sha(contract),script_sha256=sha(Path(__file__)),
    limitations=['Measured CDAC current is included; residual still contains output MOS displacement and other omitted node terms.',
...
(P/'evidence/reference-loaded-balance.json').write_text(json.dumps(out,indent=2)+'\n')
