...
def main(*, loaded=False, entrypoint=None):
    source='balance' if loaded else 'devices'
    e=P/f'evidence/sar-reference-{source}.json'
    if loaded and not e.exists():
        print('Accepted instrumented waveform pending; no current attribution performed.');sys.exit(0)
    d=json.loads(e.read_text());assert d['completed'] and d['reproduction_pass']
    p=R/f'scratch/transceiver-sar-reference-{source}/baseline.dat';assert sha(p)==d['waveform_sha256']
    ...
    for hold in (70,120,170):
        for j in range(8):
            ...
            for rail,node in [('xhigh','vh'),('xlow','vl')]:
                ...
                if loaded:
                    load_current=v('i(vsenseh)' if rail=='xhigh' else 'i(vsensel)')
                    qload=float(np.trapezoid(load_current,grid))
                    rows[-1].update(cdac_outward_charge_c=qload,pre_cdac_residual_c=qc+qm-qr,
                        unaccounted_outward_charge_c=qc+qm-qr-qload,
                        equivalent_unaccounted_current_a=(qc+qm-qr-qload)/(end-start))
    out=dict(reservoir_capacitance_f=cap,rows=rows,waveform_sha256=sha(p),probe_contract_sha256=sha(contract),script_sha256=sha(Path(entrypoint or __file__)),
        limitations=[('Measured CDAC current is included; residual still contains output MOS displacement and other omitted node terms.' if loaded else 'Residual includes CDAC current, output MOS displacement and all other omitted node terms.'),
    ...
    if entrypoint is not None:out['shared_script_sha256']=sha(Path(__file__))
    (P/f'evidence/reference-{"loaded" if loaded else "partial"}-balance.json').write_text(json.dumps(out,indent=2)+'\n')
