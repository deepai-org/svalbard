...
rows=[]
for label,folder in [('baseline_1pf','transceiver-rf-ideal-lo'),('reduced_01pf','transceiver-rf-ideal-lo-cap01')]:
...
r=dict(status='controlled_if_capacitance_comparison_not_receiver_qualification',cases=rows,limitations=['Ideal sources disconnect real mixer gate load from oscillator buffers; no equal-power or equal-loading clock comparison.', 'Fixed receive topology, input tone and sample timing; ideal LO phase differs from autonomous clock, compare tone magnitudes.', 'Only IF capacitors change; 0.1pF is an ideal scenario, not a selected physical capacitance including parasitics.', 'No intrinsic noise, ADC, quadrature, process, mismatch, PEX, EVM or blocker qualification.'])
r['held_gain_ratios_to_1pf']={row['case']:row['gains_v_per_v']['held']/rows[0]['gains_v_per_v']['held'] for row in rows[1:]}
(ROOT/'projects/programmable_transceiver_platform/evidence/rf-if-cap-comparison.json').write_text(json.dumps(r,indent=2)+'\n')
