...
if __name__ == '__main__':
    ...
    if args.smoke:
        ...
        sys.exit(0)
    files = list((P / 'system_model/connected').glob('*.py')) + [Path(__file__), Path(__file__).with_name('statistical_three_cap_quality.py'), RANKING] + list(Path(__file__).parent.glob("thermal_filter*.py"))
    ...
    path = P / f'evidence/batched-thermal-stressed-three-cap-quality-mode{args.mode}-launch.json'
    path.write_text(json.dumps(report, indent=2) + '\n')
    try:
        main(args.mode, True, actual_chip_class=StressedChip, experiment_tag='three-cap-batched-thermal-stressed-repeated-cal')
        result_path = P / f'evidence/connected-limited-rail20-pad-quality-mode{args.mode}-three-cap-batched-thermal-stressed-repeated-cal-phase-diagnostic.json'
