...
def main(variant):
    if variant not in ('mask', 'rank', 'prefix'):
    ...
    out = root / f'scratch/transceiver-block-rx-{variant}-mapping'
    ...
    for mode in (0, 1):
        ...
        assert (paths['max']['slack_ns'] > 0 if variant == 'prefix' else paths['max']['slack_ns'] < 0)
        cases[mode] = paths
    assert not s.split('ELECTRICAL_BEGIN')[1].split('ELECTRICAL_END')[0].strip()
    assert 'unconstrained endpoints' not in s
    files = [p / 'rtl' / f for f in (f'pt_block_rx_{variant}.sv', 'pt_block_rx_routed.vh', 'pt_block_header.sv', f'pt_block_route_{variant}.sv', 'pt_lane_compact.sv', 'pt_schedule.vh')] + [p / 'verification' / f for f in (f'block_rx_{variant}_timing.tcl', f'run_block_rx_{variant}_mapping.sh', f'report_block_rx_{variant}_mapping.py')] + [out / f for f in ('mapped.v', 'mapped.json', 'stat.json', 'timing.log', 'library.sha256')]
    files.append(pathlib.Path(__file__).resolve())
    r = {'pass': {'mask': 79, 'rank': 81, 'prefix': 83}[variant], 'status': ('nominal unplaced setup screen passes both modes' if variant == 'prefix' else 'setup fails both modes'), 'cell_area_um2': stat['area'], 'flip_flops': sum((n for k, n in stat['num_cells_by_type'].items() if '__dff' in k)), 'paths': cases, 'period_ns': 25.6, 'input_max_delay_ns': 4, 'output_max_delay_ns': 4, 'scope': 'Nominal TT25C3.3V unplaced ideal clocks/no wire RC. No mapped equivalence, queues or physical signoff.', 'sha256': {str(f.relative_to(root)): hashlib.sha256(f.read_bytes()).hexdigest() for f in files}}
    (p / f'evidence/block-rx-{variant}-mapping.json').write_text(json.dumps(r, indent=2) + '\n')
