"""Characterize the selected receiver mapping across the five retained corners."""
...
def main(commit=False):
    ...
    src = pathlib.Path('/src/verification/block_rx_commit_timing.tcl' if commit else '/src/verification/block_rx_prefix_timing.tcl').read_text()
    hashes = {}
    for corner in ('tt_025C_3v30', 'ff_n40C_3v60', 'ff_125C_3v60', 'ss_n40C_3v00', 'ss_125C_3v00'):
        ...
        script = out / (corner + '.tcl')
        script.write_text(src.replace('tt_025C_3v30', corner).replace('/out/mapped.v', '/mapped/mapped.v'))
        with (out / (corner + '.log')).open('w') as f:
            subprocess.run(['sta', '-exit', str(script)], stdout=f, stderr=subprocess.STDOUT, check=True)
