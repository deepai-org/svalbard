...
import numpy as np
R=Path(__file__).resolve().parents[3];P=R/'projects/programmable_transceiver_platform';W=R/'scratch/transceiver-bb-noise-large-input'
...
out=dict(completed=True,provenance=r,resistor_expected_asd_v_per_sqrt_hz=float(expected),cases=rows,limitations=['Stationary DC-linearized filter with ideal differential source,1kohm noisy source resistance per leg,CM1.177V,external bias2.25V,TT27C3.3V.','First-stage input pair W/L4u/.28u to8u/.56u; unadopted candidate. Includes source-resistor and filter noise.','PDK fnoicor0/1 scenarios are not guaranteed bounds on fab behavior.','No periodically switched mixer noise folding, LNA noise, oscillator phase noise, bias-supply noise, ADC noise or receiver sensitivity qualification.','1kHz-20MHz is a diagnostic integration band, not proof of usable Wi-Fi bandwidth or EVM.'])
(P/'evidence/bb-noise-large-input.json').write_text(json.dumps(out,indent=2)+'\n')
