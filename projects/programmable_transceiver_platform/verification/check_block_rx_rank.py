...
''')
rtl=['/src/rtl/'+f for f in ('pt_lane_compact.sv','pt_block_route_rank.sv','pt_block_header.sv','pt_block_rx_rank.sv')]
def run(files,name):
...
subprocess.run(['yosys','-Q','-T','-p','read_verilog -sv -I/src/rtl '+' '.join(rtl)+'; synth -top pt_block_rx_rank; check -assert'],stdout=(out/'synthesis.log').open('w'),check=True)
