...
import numpy as np
R=Path(__file__).resolve().parents[3];P=R/'projects/programmable_transceiver_platform';W=R/'scratch/transceiver-lo-mixer-sine'
...
if (W/'result.json').exists():
 ...
 for c in r['cases']:
  ...
  deck=(W/(n+'.spice')).read_text();start=deck.index('.include /wifi/rf_switch_mixer/mixer.spice');end=deck.index('CN MN 0 1p',start)+len('CN MN 0 1p\n')
  assert deck[:start]+deck[end:]==(R/'scratch/transceiver-lo-selfbias-sine'/(n+'.spice')).read_text()
...
out['limitations']=['Complementary AC-coupled self-biased drivers with actual mixer and1k/1p IF loads; ideal RF1.5V through300ohm, no LNA/filter/actual ring modulation.', 'Passing this screen cannot qualify the autonomous receiver or identify a unique cause of its missing pulses.']
(P/'evidence/lo-mixer-sine.json').write_text(json.dumps(out,indent=2)+'\n');print(out['status'],out['completed']);print(out['cases'])
