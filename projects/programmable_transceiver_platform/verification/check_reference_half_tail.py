...
import numpy as np
R=Path(__file__).resolve().parents[3];P=R/'projects/programmable_transceiver_platform';W=R/'scratch/transceiver-reference-pair-half-tail-dc';B=R/'scratch/transceiver-reference-pair-device-dc'
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
m=json.loads((W/'change-manifest.json').read_text());assert len(m['changes'])==1
for old,new in m['changes']:
 assert old.startswith('XT ') and 'pfet_03v3' in old
 assert new==old.replace('m={16*S}','m={8*S}')
...
out=dict(disposition="Diagnostic only: assess tail headroom before any dynamic qualification; retained baseline unchanged.",completed=True,exact_declared_changes_verified=True,cases=rows,provenance=records,limitations=['DC only; Reduced tail multiplicity changes current, gain and capacitance; compensation and connected dynamics must be rechecked.','Changing tail multiplicity changes operating current and capacitance as well as headroom; not a unique causal proof.','Ideal target/bias and zero external load remain.'])
(P/'evidence/reference-half-tail.json').write_text(json.dumps(out,indent=2)+'\n');print(rows)
