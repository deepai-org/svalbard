...
import numpy as np
R=Path(__file__).resolve().parents[3];P=R/'projects/programmable_transceiver_platform';W=R/'scratch/transceiver-reference-pair-hybrid-dc';B=R/'scratch/transceiver-reference-pair-device-dc'
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
m=json.loads((W/'change-manifest.json').read_text());pair=(P/'analog/reference/adc_reference_pair_tuned.spice').read_text()
...
for name,target in [('VH',2.15),('VL',1.15)]:
 ...
 for label,a in [('baseline',b),('candidate',c)]:
  ...
  values.append(dict(case=label,error_v=float(v[node]-target),mirror_current_difference_a=float(v[stem+'xmn.m0[id]']-v[stem+'xmp.m0[id]']),tail_margin_v=float(v[stem+'xt.m0[vds]']-v[stem+'xt.m0[vdsat]']),device_margin_sweep_min_v={dev:float((a[:,h.index(stem+dev+'.m0[vds]')]-a[:,h.index(stem+dev+'.m0[vdsat]')]).min()) for dev in ('xip','xin','xt','xmp','xmn','xout','xload')},supply_power_w=float(-3.3*v['i(vdd)'])))
 rows.append(dict(rail=name,values=values))
out=dict(disposition="Diagnostic candidate only; DC headroom is not dynamic regulation.",completed=True,exact_declared_changes_verified=True,cases=rows,provenance=records,limitations=['DC only; changed input topology changes internal swing and loop response; compensation and connected dynamics must be rechecked.','High input stage changes polarity and bias; device operating points must be checked before dynamic promotion.','Ideal target/bias and zero external load remain.'])
(P/'evidence/reference-hybrid.json').write_text(json.dumps(out,indent=2)+'\n');print(rows)
