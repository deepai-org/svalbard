...
import numpy as np
R=Path(__file__).resolve().parents[3];P=R/'projects/programmable_transceiver_platform';W=R/'scratch/transceiver-reference-hybrid-impedance'
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
r=json.loads((W/'result.json').read_text());assert {x['name'] for x in r['cases']}=={'baseline_h','baseline_l','hybrid_h','hybrid_l'};rows=[]
for c in r['cases']:
...
(P/'evidence/reference-hybrid-impedance.json').write_text(json.dumps(out,indent=2)+'\n');print(rows)
