...
import numpy as np
R=Path(__file__).resolve().parents[3];P=R/'projects/programmable_transceiver_platform';W=R/'scratch/transceiver-reference-hybrid-step'
...
if (W/'result.json').exists():
 ...
 for c in r['cases']:
  ...
  parent=R/'scratch/transceiver-reference-hybrid-impedance'/('hybrid_'+c['rail']+'.spice')
...
(P/'evidence/reference-hybrid-step.json').write_text(json.dumps(out,indent=2)+'\n');print(out['status'],out['completed'])
for c in out['cases']:
