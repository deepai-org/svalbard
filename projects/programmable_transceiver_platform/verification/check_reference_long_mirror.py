...
import numpy as np
R=Path(__file__).resolve().parents[3];P=R/'projects/programmable_transceiver_platform';W=R/'scratch/transceiver-reference-pair-long-mirror-dc';B=R/'scratch/transceiver-reference-pair-device-dc'
...
for name,target in [('VH',2.15),('VL',1.15)]:
 ...
 for label,a in [('baseline',b),('candidate',c)]:
  ...
  values.append(dict(case=label,error_v=float(v[node]-target),mirror_current_difference_a=float(v[stem+'xmn.m0[id]']-v[stem+'xmp.m0[id]']),tail_margin_v=float(v[stem+'xt.m0[vds]']-v[stem+'xt.m0[vdsat]']),supply_power_w=float(-3.3*v['i(vdd)'])))
 rows.append(dict(rail=name,values=values))
out=dict(completed=True,exact_declared_changes_verified=True,cases=rows,provenance=records,limitations=['DC only; longer/wider devices increase capacitance and area; compensation and connected dynamics must be rechecked.','Changing length affects more than output resistance; not a unique causal proof.','Ideal target/bias and zero external load remain.'])
(P/'evidence/reference-long-mirror.json').write_text(json.dumps(out,indent=2)+'\n');print(rows)
