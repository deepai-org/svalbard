...
def main(hybrid=False):
 R=Path(__file__).resolve().parents[3];P=R/'projects/programmable_transceiver_platform';W=R/('scratch/transceiver-reference-hybrid-impedance' if hybrid else 'scratch/transceiver-reference-pair-impedance')
 r=json.loads((W/'result.json').read_text());assert {x['name'] for x in r['cases']}==({'baseline_h','baseline_l','hybrid_h','hybrid_l'} if hybrid else {'baseline_h','baseline_l','long_mirror_h','long_mirror_l'});rows=[]
 for c in r['cases']:
