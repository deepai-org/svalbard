...
for label,amplitude,source in [('400',.4,'sar-reference-clamped'),('100',.1,'sar-amplitude-bank1')]:
 B=R/f'scratch/transceiver-sar-driver-damping-{label}-prepared';W=R/f'scratch/transceiver-sar-driver-damping-{label}';m=json.loads((B/'manifest.json').read_text());assert sha(B/'baseline.spice')==m['artifacts_sha256']['baseline.spice']
 s=(B/'baseline.spice').read_text()
 cell=(P/'analog/adc/sample_driver_headroom.spice').read_text().replace('RC X Z 100','RC X Z 2k').rstrip()
 assert s.count(cell)==1;s=s.replace(cell,'.include /screen/adc/sample_driver_headroom.spice')
 ...
 donor=R/('scratch/transceiver-'+source);dr=json.loads((donor/'result.json').read_text());assert dr['returncode']==0 and not dr['timed_out'];assert sha(donor/'baseline.dat')==dr['artifacts_sha256']['.dat']
 with (donor/'baseline.dat').open() as f:dh=f.readline().lower().split()
 da=np.loadtxt(donor/'baseline.dat',skiprows=1);dt=da[:,0];reference=[]
 for hold,target in [(70,amplitude),(120,-amplitude),(170,amplitude)]:
  def dv(n,ns):return float(np.interp(ns*1e-9,dt,da[:,dh.index('v('+n+')')]))
  bits=[dv('d'+str(k),hold+39) for k in range(8)];assert all(v<.33 or v>2.97 for v in bits)
  code=sum(int(v>1.65)*2**k for k,v in enumerate(bits))
  reference.append(dict(hold_ns=hold,final_code=code,code_error=code-math.floor(128-128*target),held_error_at_clock_v=dv('hp',hold+.5)-dv('hn',hold+.5)-target))
 row['baseline_frames']=reference;row['baseline_waveform_sha256']=sha(donor/'baseline.dat')
 rows.append(row)
...
for r in rows:print(r['amplitude_v'],r['status'],[f['final_code'] for f in r.get('frames',[])])
