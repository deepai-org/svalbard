...
def sampler_frames(a, h, amplitude):
    t = a[:, 0]
    assert len(h) == a.shape[1] and np.isfinite(a).all() and np.all(np.diff(t) > 0) and (t[-1] >= 2.099e-07 - 1e-20)
    for n, v in [('vh', 2.15), ('vl', 1.15)]:
        assert np.max(abs(a[:, h.index('v(' + n + ')')] - v)) <= 1e-09

    def at(n, ns):
        return float(np.interp(ns * 1e-09, t, a[:, h.index('v(' + n + ')')]))
    frames = []
    for hold, target in [(70, amplitude), (120, -amplitude), (170, amplitude)]:
        bits = [at('d' + str(k), hold + 39) for k in range(8)]
        valid = all((v < 0.33 or v > 2.97 for v in bits))
        code = sum((int(v > 1.65) * 2 ** k for k, v in enumerate(bits)))
        ideal = math.floor(128 - 128 * target)
        points = [dict(offset_ns=dt, driver_error_v=at('ip', hold + dt) - at('in', hold + dt) - target, held_error_v=at('hp', hold + dt) - at('hn', hold + dt) - target) for dt in (0, 0.1, 0.5)]
        decisions = []
        for j in range(8):
            q = at('qp', hold + 2.4 + 5 * j) - at('qn', hold + 2.4 + 5 * j)
            res = at('hp', hold + 0.5 + 5 * j) - at('hn', hold + 0.5 + 5 * j)
...
def main():
    ...
    for banks, source in [(1, 'sar-reference-clamped'), (2, 'sar-sampler-double')]:
        B = R / f'scratch/transceiver-sar-amplitude-bank{banks}-prepared'
        W = R / f'scratch/transceiver-sar-amplitude-bank{banks}'
        m = json.loads((B / 'manifest.json').read_text())
        assert sha(B / 'baseline.spice') == m['artifacts_sha256']['baseline.spice']
        s = (B / 'baseline.spice').read_text()
        for old, new in m['input_changes']:
        ...
        if (W / 'result.json').exists():
            ...
            if r['returncode'] == 0 and (not r['timed_out']) and (not errors):
                ...
                frames = sampler_frames(a, h, 0.1)
                row.update(completed=True, frames=frames, waveform_sha256=sha(W / 'baseline.dat'), result_sha256=sha(W / 'result.json'))
        rows.append(row)
    s1 = (R / 'scratch/transceiver-sar-amplitude-bank1-prepared/baseline.spice').read_text()
    s2 = (R / 'scratch/transceiver-sar-amplitude-bank2-prepared/baseline.spice').read_text()
    assert s2.replace('XS_EXTRA IP IN HP HN SC SCB VDD 0 wifi_if_transmission_gate\n', '') == s1
    ...
    for r in rows:
        print(r['banks'], r['status'], [f['final_code'] for f in r.get('frames', [])])
