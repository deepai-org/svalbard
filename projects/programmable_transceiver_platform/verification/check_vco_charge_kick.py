...
import numpy as np
R=Path(__file__).resolve().parents[3];P=R/'projects/programmable_transceiver_platform';W=R/('scratch/transceiver-vco-charge-kick-fine' if args.fine else 'scratch/transceiver-vco-charge-kick-v2');B=R/'scratch/transceiver-vco-split-tuning/v1.08.spice'
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
m=json.loads((W/'manifest.json').read_text());assert sha(B)==m['baseline_deck_sha256'] and m['pulse_nodes']==['XRX.CP','XRX.CN']
...
for name,amplitude in [('quiet',0),('positive',1e-4),('negative',-1e-4)]:
 ...
 pulse=f'IKICK XRX.CP XRX.CN PWL(0n 0 20n 0 20.001n {amplitude} 20.011n {amplitude} 20.012n 0)\n'
...
out=dict(completed=terminal and all(x['completed'] for x in rows),cases=rows,source_hashes_final_verified=terminal,
 limitations=['One injection phase/amplitude at isolating output; not internal-source phase sensitivity or intrinsic noise.',
...
(P/'evidence'/('vco-charge-kick-fine.json' if args.fine else 'vco-charge-kick.json')).write_text(json.dumps(out,indent=2)+'\n');print([(x['name'],x['status'],x.get('windows')) for x in rows])
