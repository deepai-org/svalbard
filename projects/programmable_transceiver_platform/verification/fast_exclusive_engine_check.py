...
def main():
    ...
    c=ExclusiveEngineChip(watchdog_s=1e-3);reject(lambda:c.configure(0,0.))
    rows=[]
    ...
    out=dict(status='passed',source_sha256={str(f.relative_to(P)):hashlib.sha256(f.read_bytes()).hexdigest() for f in files},cases=rows,
        ...
        'Python selection API; serialized management/RTL interlock not implemented.',
        'RF capture and wired receive checked; full selected-engine duplex and calibration validity across mode changes need further coverage.'])
