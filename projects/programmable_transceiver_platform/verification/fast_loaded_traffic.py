...
def main():
    ...
    report=dict(status='running',exclusive_rf=args.exclusive,source_sha256=hashes,cases=[],physical_qualification=False,
        limitations=['Transport and event alignment only; loaded RF waveform quality and calibration accuracy remain unqualified.',
                     ...
                     'Finite count-free stream with lossy stop; no indefinite service guarantee. Inactive clock/bias shutdown remains open.'])
    output=P/('evidence/fast-exclusive-rf-traffic.json' if args.exclusive else 'evidence/fast-loaded-traffic.json');start=time.monotonic()
    ...
    try:
        for mode in (0,1):
            chips=[]
            def factory(**kwargs):
                c=(PreparedExclusiveChip if args.exclusive else PreparedLoadedChip)(**kwargs);chips.append(c);return c
