...
def simulate(mode,impaired):
    ...
    class Observed(PreparedChip):
        def __init__(self,**kwargs):
            ...
            kwargs['dac_latency_s']=profile['converter_latency_periods']['dac']*period
            kwargs['adc_latency_s']=profile['converter_latency_periods']['adc']*period
            options=dict(load_capacitance=0,output_parameters=dict(
                gain_imbalance_db=0.,phase_error_deg=0.,lo_feedthrough=0j,cubic=0.))
            if impaired:
                options=dict(load_capacitance=1e-12,dac_reference_load_capacitance=profile['shared_reference']['dac_load_f'],
...
def main():
    ...
    report=dict(status='running',source_sha256=hashes,cases=[],physical_qualification=False,
        limitations=['Finite continuous-service record with timed lossy stop, not indefinite service bounds.',
                     ...
                     'Two 0.1-amplitude blockers at 20/30 MHz in fixed 2.4 GHz envelope frame; cubic coefficient 0.05.',
                     'Selected nominal carrier only; arbitrary blocker/carrier placement remains unqualified.',
                     'Profile DAC reference load 2 pF and ADC/DAC latency 2/1.5 sample periods; other assumptions retain the fast blocker fixture.'])
    output=p/'evidence/fast-profile-load-quality.json'
