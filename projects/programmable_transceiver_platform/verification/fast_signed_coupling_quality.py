...
def simulate(mode,impaired,sign=1):
...
def main():
    ...
    output=p/'evidence/fast-signed-coupling-quality.json'
    ...
    try:
        for mode in (0,1):
            ...
            for sign in (-1,1):
                rx,tx,t,traffic=simulate(mode,True,sign)
                assert t==t0 and len(rx)==len(rx0)
                rq=quality(rx0,rx);tq=quality(tx0,tx)
                report['cases'].append(dict(mode=mode,sign=sign,rx_quality=rq,tx_quality=tq,traffic=traffic));save()
