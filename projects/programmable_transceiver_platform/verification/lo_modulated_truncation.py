...
def run(period,subdivision=1):
    carrier=2.4e9;cutoff=10e6;pole=2*np.pi*cutoff
    ...
    truth=lfilter([1.],[1.,-decay],injection)
        ...
        for offset,d,i in terms:
            for amp,f in zip(amplitudes,frequencies):
                for value,frequency in ((d*amp,offset+f),(i*amp.conjugate(),offset-f)):
                    ...
                    estimated+=value*pole/(pole+rate)*(np.exp(rate*t)-np.exp(-pole*t))
...
def main():
    ...
    result=dict(status='completed',numerical_checks='passed',approximation_screen_pass=all(
                    c[grid]['approximation_screen_pass'] for c in cases for grid in ('coarse','fine')),source_sha256=hashes,cases=cases,carrier_hz=2.4e9,cutoff_hz=10e6,
                ...
                'First-order filter, normalized switching drive; no gate-voltage mapping.',
                'Errors measure envelope truncation, not total RF quality or noise.'])
    (P/'evidence/lo-modulated-truncation.json').write_text(json.dumps(result,indent=2)+'\n')
