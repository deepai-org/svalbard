...
root=pathlib.Path(__file__).resolve().parents[3];p=root/'projects/programmable_transceiver_platform';out=root/'scratch/transceiver-block-rx-prefix-mapping'
...
files=[p/'rtl'/f for f in ('pt_block_rx_prefix.sv','pt_block_header.sv','pt_block_route_prefix.sv','pt_lane_compact.sv','pt_schedule.vh')]+[p/'verification'/f for f in ('block_rx_prefix_timing.tcl','run_block_rx_prefix_mapping.sh','report_block_rx_prefix_mapping.py')]+[out/f for f in ('mapped.v','mapped.json','stat.json','timing.log','library.sha256')]
r={'pass':83,'status':'nominal unplaced setup screen passes both modes','cell_area_um2':stat['area'],'flip_flops':sum(n for k,n in stat['num_cells_by_type'].items() if '__dff' in k),'paths':cases,'period_ns':25.6,'input_max_delay_ns':4,'output_max_delay_ns':4,'scope':'Nominal TT25C3.3V unplaced ideal clocks/no wire RC. No mapped equivalence, queues or physical signoff.','sha256':{str(f.relative_to(root)):hashlib.sha256(f.read_bytes()).hexdigest() for f in files}}
(p/'evidence/block-rx-prefix-mapping.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps({k:v for k,v in r.items() if k!='sha256'},indent=2))
