...
for diff in [-.4,-.001,.001,.4]:
 for clocked in [False,True]:
  name=f'd{diff:g}_clk{int(clocked)}';s=base
...
rows=[]
for diff in [-.4,-.001,.001,.4]:
 a=arrays[f'd{diff:g}_clk1'];b=arrays[f'd{diff:g}_clk0']
 # Input-source current sign retained explicitly; integrate excess current at clamped inputs.
 lo,hi=2e-9,4.9e-9;t=np.r_[lo,a[(a[:,0]>lo)&(a[:,0]<hi),0],hi]
 current=np.column_stack([np.interp(t,a[:,0],a[:,col])-np.interp(t,b[:,0],b[:,col]) for col in [1,2]])
 charge=np.trapezoid(current,t,axis=0)
 rows.append(dict(differential_v=diff,window_ns=[2,4.9],source_charge_c=charge.tolist(),differential_source_charge_c=float(charge[0]-charge[1]),peak_differential_current_a=float(np.max(abs(current[:,0]-current[:,1]))),output_difference_v=float(np.interp(4.9e-9,a[:,0],a[:,3]-a[:,4]))))
