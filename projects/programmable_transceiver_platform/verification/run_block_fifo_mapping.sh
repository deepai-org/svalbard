...
yosys -Q -T -p "read_verilog -sv /src/rtl/pt_fifo.sv /src/rtl/pt_block_fifo.sv; synth -top pt_block_fifo -flatten; dfflibmap -liberty $LIB; abc -liberty $LIB -D 25600 -constr /out/abc.constr; read_liberty -lib $LIB; clean; check -assert; tee -o /out/stat.json stat -json -liberty $LIB; write_verilog -noattr /out/mapped.v; write_json /out/mapped.json" > /out/synthesis.log
sta -exit /src/verification/block_fifo_timing.tcl > /out/timing.log 2>&1'
