...
OUT="$ROOT/scratch/transceiver-latest-rf-loop-extended"
mkdir -p "$OUT"
docker run --rm --platform linux/arm64 --network none --cpus 2 --memory 4g --entrypoint /bin/bash \
 ...
 -v "$ROOT/scratch/transceiver-latest-rf-loop-extended-prepared:/baseline:ro" \
 -v "$OUT:/work" --workdir /work \
 sha256:bd7a702bef0b85f5ebf67efca449f270fbeb185380ead204559fcd2457959305 \
 -lc 'python3 /screen/pll/closed_loop_buffered_screen.py --latest "$@"' runner "$@"
