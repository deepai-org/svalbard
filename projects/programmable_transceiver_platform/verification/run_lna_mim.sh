...
MODE="${1:?Use clean, lossy, or inductive}"
case "$MODE" in clean|lossy|inductive) ;; *) exit 2 ;; esac
OUT="$ROOT/scratch/transceiver-lna-mim-$MODE"
mkdir -p "$OUT"
docker run --rm --platform linux/arm64 --network none --cpus 2 --memory 4g --entrypoint /bin/bash \
 ...
 -lc "python3 /screen/lna/noise_screen.py --mim=$MODE"
