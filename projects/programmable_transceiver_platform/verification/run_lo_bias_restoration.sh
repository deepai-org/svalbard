...
OUT="$ROOT/scratch/transceiver-lo-bias-restoration"
mkdir "$OUT"
docker run --rm --name transceiver-lo-bias-restoration --platform linux/arm64 --network none --cpus 2 --memory 4g --entrypoint /bin/bash \
 -v "$ROOT/projects/programmable_transceiver_platform/analog:/screen:ro" \
 -v "$ROOT/ip/blocks/analog/wifi_80211b:/wifi:ro" \
 -v "$ROOT/scratch/transceiver-lo-bias-restoration-prepared:/prepared:ro" \
