...
OUT="$ROOT/scratch/transceiver-lo-rf-only"
mkdir "$OUT"
docker run --rm --name transceiver-lo-rf-only --cidfile "$OUT/container.id" --platform linux/arm64 --network none --cpus 2 --memory 8g --entrypoint /bin/bash \
 ...
 -v "$ROOT/scratch/transceiver-lo-rf-only-prepared:/prepared:ro" \
