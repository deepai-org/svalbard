...
OUT="$ROOT/scratch/transceiver-quadrature-lna-ac"
mkdir -p "$OUT"
docker run --rm --platform linux/arm64 --network none --cpus 2 --memory 4g --entrypoint /bin/bash \
 ...
 -v "$ROOT/scratch/transceiver-quadrature-ring:/baseline:ro" \
 ...
 -lc 'python3 /screen/quadrature/mixer_screen.py --lna --final-stage --ac-coupled'
