...
OUT="$ROOT/scratch/transceiver-reference-load-startup"
mkdir -p "$OUT"
docker run --rm --platform linux/arm64 --network none --cpus 2 --memory 4g --entrypoint /bin/bash \
 ...
 -v "$ROOT/scratch/transceiver-reference-load-prepared:/baseline:ro" \
 ...
 -lc 'python3 /screen/reference/load_startup_screen.py'
