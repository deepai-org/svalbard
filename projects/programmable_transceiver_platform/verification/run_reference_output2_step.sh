...
OUT="$ROOT/scratch/transceiver-reference-hybrid-output2-step"
mkdir "$OUT"
docker run --rm --platform linux/arm64 --network none --cpus 2 --memory 4g --entrypoint /bin/bash \
 ...
 -v "$ROOT/scratch/transceiver-reference-output2-impedance:/baseline:ro" \
 ...
 -lc 'python3 /screen/reference/pair_hybrid_step.py'
