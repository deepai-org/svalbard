...
OUT="$ROOT/scratch/transceiver-reference-terminal-probe"
mkdir "$OUT"
docker run --rm --name transceiver-reference-terminal-probe --platform linux/arm64 --network none --cpus 1 --memory 4g --entrypoint /bin/bash \
 -v "$ROOT/projects/programmable_transceiver_platform/analog:/screen:ro" \
 -v "$ROOT/ip/blocks/analog/wifi_80211b:/wifi:ro" \
 -v "$ROOT/scratch/transceiver-reference-terminal-probe-prepared:/prepared:ro" \
