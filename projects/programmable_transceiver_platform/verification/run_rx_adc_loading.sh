...
OUT="$ROOT/scratch/transceiver-rx-adc-loading"
...
docker run --rm --platform linux/arm64 --network none --cpus 2 --memory 4g --entrypoint /bin/bash \
 ...
 -v "$ROOT/scratch/transceiver-rx-adc-preflight-dedup:/baseline:ro" \
 ...
 -lc 'python3 /screen/integration/rx_adc_loading.py "$@"' runner "$@"
