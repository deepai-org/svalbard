...
OUT="$ROOT/scratch/transceiver-sar-driver-physical-400"
mkdir "$OUT"
docker run --rm --name transceiver-sar-driver-physical-400 --platform linux/arm64 --network none --cpus 1 --memory 4g --entrypoint /bin/bash \
 -v "$ROOT/projects/programmable_transceiver_platform/analog:/screen:ro" \
 -v "$ROOT/ip/blocks/analog/wifi_80211b:/wifi:ro" \
 -v "$ROOT/scratch/transceiver-sar-driver-physical-400-prepared:/prepared:ro" \
