...
OUT="$ROOT/scratch/transceiver-sar-known-load"
mkdir "$OUT"
docker run --rm --name transceiver-sar-known-load --platform linux/arm64 --network none --cpus 1 --memory 4g --entrypoint /bin/bash \
 -v "$ROOT/projects/programmable_transceiver_platform/analog:/screen:ro" \
 -v "$ROOT/ip/blocks/analog/wifi_80211b:/wifi:ro" \
 -v "$ROOT/scratch/transceiver-sar-known-load-prepared:/prepared:ro" \
