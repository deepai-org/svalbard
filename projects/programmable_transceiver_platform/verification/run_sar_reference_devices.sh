...
OUT="$ROOT/scratch/transceiver-sar-reference-devices"
mkdir "$OUT"
docker run --rm --name transceiver-sar-reference-devices --platform linux/arm64 --network none --cpus 1 --memory 4g --entrypoint /bin/bash \
 -v "$ROOT/projects/programmable_transceiver_platform/analog:/screen:ro" \
 -v "$ROOT/ip/blocks/analog/wifi_80211b:/wifi:ro" \
 -v "$ROOT/scratch/transceiver-sar-reference-devices-prepared:/prepared:ro" \
